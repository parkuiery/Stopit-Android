...
class ReviewPromptPostReleaseFollowthroughDocsTest(unittest.TestCase):
    def test_followthrough_runbook_preserves_release_and_measurement_boundary(self):
        ...
        self.assertIn("2026-06-25T21:14:08Z metrics snapshot smoke", followthrough)
        self.assertIn("review_prompt_skipped`는 `54 users / 97 events`", followthrough)
        self.assertIn("first_core_action_completed 518 users", followthrough)
