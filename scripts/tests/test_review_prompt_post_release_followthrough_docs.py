...
class ReviewPromptPostReleaseFollowthroughDocsTest(unittest.TestCase):
    def test_followthrough_runbook_preserves_release_and_measurement_boundary(self):
        ...
        self.assertIn("2026-06-26T02:07:51Z metrics snapshot smoke", followthrough)
        self.assertIn("review_prompt_skipped`는 `56 users / 100 events`", followthrough)
        self.assertIn("first_core_action_completed 523 users", followthrough)
