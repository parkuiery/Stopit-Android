...
## Purpose
Broadcast receivers for boot restoration and routine alarm dispatch.

## Key Files
| File | Description |
|------|-------------|
| `BootReceiver.kt` | Restores schedules or state after device boot. |
| `RoutineAlarmReceiver.kt` | Handles routine alarm broadcasts and starts/stops routine behavior. |
