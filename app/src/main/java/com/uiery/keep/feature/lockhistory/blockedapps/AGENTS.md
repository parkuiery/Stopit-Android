...
## Purpose
Detail route for apps blocked in a selected history session.

## Key Files
| File | Description |
|------|-------------|
| `BlockedAppsNavigation.kt` | Type-safe navigation route and graph wiring for BlockedApps. |
| `BlockedAppsScreen.kt` | Compose screen for the BlockedApps flow. |
| `BlockedAppsViewModel.kt` | Orbit MVI view-model/state holder for BlockedApps. |
