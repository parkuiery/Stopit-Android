...
## Purpose
Developer-tool feature for debug/dev builds and internal diagnostics.

## Key Files
| File | Description |
|------|-------------|
| `DevToolNavigation.kt` | Type-safe navigation route and graph wiring for DevTool. |
| `DevToolScreen.kt` | Compose screen for the DevTool flow. |
| `DevToolViewModel.kt` | Orbit MVI view-model/state holder for DevTool. |

## Subdirectories
| Directory | Purpose |
|-----------|---------|
| `component/` | Developer-tool list item composables. (see `component/AGENTS.md`) |
