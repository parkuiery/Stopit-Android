...
### 2026-06-07T16:13:51Z metrics snapshot readback: 30일 합산 guardrail

- 확인 명령: `python3 /Users/uiel/.hermes/scripts/stopit_metrics_snapshot.py`
- window: `30daysAgo..yesterday`
- 확인된 30일 screen quality:
  - total `screen_view`: `41,792`
  - `(not set)+blank` `unifiedScreenName` gap: `26,138`
  - `(not set)` share: `26,138 / 41,792 = 62.5%`
  - visible top screens: `BlockScreen` `8,087`, `HomeScreen` `3,580`, `RoutineScreen` `729`, `OnboardingIntroScreen` `853`, `SplashScreen` `1,009`, `MenuScreen` `513`, `LockScreen` `298`
- latest observed production version adoption smoke:
  - `appVersion=1.7.7` activeUsers: `199`
  - total activeUsers: `792`
  - latest-version active share: `199 / 792 = 25.1%` → `docs/VERSION_ADOPTION_METRICS_GATE.md` 기준 `주의`
...
| `(not set)+빈 값` 비율 | `78.1%` | `TODO` | 2026-05-29 baseline은 PR #296의 `SplashScreen` / `BlockedAppsScreen` / `EmergencyUnlockSettingsScreen` 및 PR #318의 dev/debug `DevToolScreen` 보강 전 기준선. 2026-06-03 14일 live smoke는 `13,780 / 22,584 = 61.0%`였고, 2026-06-07 30일 cron snapshot은 `(not set)+blank` `26,138 / 41,792 = 62.5%`였다. 두 값 모두 PR #296/#318이 `origin/main`/`v1.7.7`에 없고 최신 `1.7.7` active share도 `199 / 792 = 25.1%`라 post-fix 성과가 아니라 중간 smoke/주의 상태로만 기록한다. PR #296/#318 포함 버전 배포 후 14일 창에서 재측정. `DevToolScreen`은 dev/debug 내부 진단 surface로 production 사용자 screen denominator와 분리. |
