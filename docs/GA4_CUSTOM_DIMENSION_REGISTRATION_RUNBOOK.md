...
| `period_type` | `lock_history_performance_summary_viewed`, `lock_history_top_apps_viewed` | #465 UI/read model 구현은 PR #485로 develop 반영 / 이벤트 코드는 후속 후보 | 후속 instrumentation을 추가한 버전 배포 전후로 GA4 Admin 등록 후 metadata 확인 | `customEvent:period_type` |
| `report_state` | `lock_history_performance_summary_viewed` | #465 UI/read model 구현은 PR #485로 develop 반영 / 이벤트 코드는 후속 후보 | empty/low-data/has-history 상태별 summary 노출 비교. 실패/중독 상태값 금지 | `customEvent:report_state` |
| `session_count_bucket` | `lock_history_performance_summary_viewed` | #465 UI/read model 구현은 PR #485로 develop 반영 / 이벤트 코드는 후속 후보 | raw session 목록/개별 timestamp 없이 count bucket만 사용 | `customEvent:session_count_bucket` |
| `duration_minutes_bucket` | `lock_history_performance_summary_viewed` | #465 UI/read model 구현은 PR #485로 develop 반영 / 이벤트 코드는 후속 후보 | raw duration 값 대신 bucket만 사용 | `customEvent:duration_minutes_bucket` |
| `top_apps_count_bucket` | `lock_history_top_apps_viewed` | #465 UI/read model 구현은 PR #485로 develop 반영 / 이벤트 코드는 후속 후보 | top app 이름/package 원문 금지. 표시 개수 bucket만 사용 | `customEvent:top_apps_count_bucket` |
...
### 2026-06-05T14:26:22Z metrics snapshot readback: 30일 합산 guardrail

- 확인 명령: `python3 /Users/uiel/.hermes/scripts/stopit_metrics_snapshot.py`
- window: `30daysAgo..yesterday`
- 확인된 30일 screen quality:
  - total `screen_view`: `37,013`
  - `(not set)` `unifiedScreenName`: `24,075`
  - `(not set)` share: `24,075 / 37,013 = 65.0%`
...
- latest observed production version adoption smoke:
  - `appVersion=1.7.7` activeUsers: `109`
  - total activeUsers: `762`
  - latest-version active share: `109 / 762 = 14.3%` → `docs/VERSION_ADOPTION_METRICS_GATE.md` 기준 `주의`
...
| `(not set)+빈 값` 비율 | `78.1%` | `TODO` | 2026-05-29 baseline은 PR #296의 `SplashScreen` / `BlockedAppsScreen` / `EmergencyUnlockSettingsScreen` 및 PR #318의 dev/debug `DevToolScreen` 보강 전 기준선. 2026-06-03 14일 live smoke는 `13,780 / 22,584 = 61.0%`였고, 2026-06-05 30일 cron snapshot은 `(not set)` `24,075 / 37,013 = 65.0%`였다. 두 값 모두 PR #296/#318이 `origin/main`/`v1.7.7`에 없고 최신 `1.7.7` active share도 `109 / 762 = 14.3%`라 post-fix 성과가 아니라 중간 smoke/주의 상태로만 기록한다. PR #296/#318 포함 버전 배포 후 14일 창에서 재측정. `DevToolScreen`은 production 사용자 지표와 분리 |
