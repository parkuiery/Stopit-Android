...
| `lock_mode` | `goal_lock_created`, `goal_lock_completed`, `goal_lock_ended_early`, `goal_lock_updated` | `goal_lock_created` 코드 계약 추가, detail 종료 path의 early-end runtime call, completion runtime call, detail duration/lock-mode update runtime call 추가 / release·GA4 등록 전 | 동일 | `customEvent:lock_mode` |
...
### 2026-06-11T00:10:19Z metrics snapshot readback: 30일 합산 guardrail

- 확인 명령: `python3 /Users/uiel/.hermes/scripts/stopit_metrics_snapshot.py`
- window: `30daysAgo..yesterday`
- 확인된 30일 screen quality:
  - total `screen_view`: `46,694`
  - `(not set)+blank` `unifiedScreenName` gap: `28,316`
  - `(not set)+blank` share: `28,316 / 46,694 = 60.6%`
  - visible top screens: `BlockScreen` `10,019`, `HomeScreen` `3,869`, `RoutineScreen` `809`, `OnboardingIntroScreen` `894`, `SplashScreen` `1,009`, `MenuScreen` `674`, `LockScreen` `381`
- latest observed production version adoption smoke:
  - `appVersion=1.7.7` activeUsers: `267`
  - total activeUsers: `821`
  - latest-version active share: `267 / 821 = 32.5%` → `docs/VERSION_ADOPTION_METRICS_GATE.md` 기준 `충분`
...
| `(not set)+빈 값` 비율 | `78.1%` | `TODO` | 2026-05-29 baseline은 PR #296의 `SplashScreen` / `BlockedAppsScreen` / `EmergencyUnlockSettingsScreen` 및 PR #318의 dev/debug `DevToolScreen` 보강 전 기준선이고, PR #358 merge commit `6ceaecc4`가 이 release-boundary를 고정했다. 2026-06-03 14일 live smoke는 `13,780 / 22,584 = 61.0%`였고, 2026-06-11T00:10:19Z 30일 metrics snapshot은 `(not set)+blank` `28,316 / 46,694 = 60.6%`였다. 최신 `1.7.7` active share는 `267 / 821 = 32.5%`로 충분 상태에 들어왔지만 `PR #296/#318/#358` package와 PR #755 Firebase `screen_view` backend payload 보강이 `origin/main`/`v1.7.7`에 없으므로 **post-fix 성과가 아니라 release boundary 전 중간 smoke**로만 기록한다. 화면 호출 coverage package와 backend payload package가 모두 포함된 버전 배포 후 14일 창에서 **D+14 screen quality 재측정**. `DevToolScreen`은 dev/debug 내부 진단 surface로 production 사용자 screen denominator와 분리. |
