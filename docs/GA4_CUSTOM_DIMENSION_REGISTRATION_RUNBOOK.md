...
- `docs/REPEAT_BLOCK_ROUTINE_SUGGESTION.md`(#531)의 반복 차단 기반 자동 루틴 제안이 구현된 뒤, 어떤 반복 패턴이 추천 노출·클릭·적용으로 이어지는지 privacy-safe enum/bucket 기준으로만 비교하기 위한 최소 집합이다.
...
- #531 문서/구현 PR이 event dictionary를 갱신하더라도, GA4 Admin 등록·metadata 확인·추천 포함 release/tag/Play deploy·14일 관측 전에는 event 0건을 수요 없음이나 추천 UX 실패로 해석하지 않는다.
...
| `period_type` | `lock_history_performance_summary_viewed`, `lock_history_top_apps_viewed` | #465 UI/read model PR #485 develop 반영 / 2026-06-05 code-lane instrumentation 추가 / GA4 등록 필요 | instrumentation 포함 버전 배포 전후로 GA4 Admin 등록 후 metadata 확인 | `customEvent:period_type` |
...
| `suggestion_reason` | `repeat_block_routine_suggestion_shown`, `repeat_block_routine_suggestion_clicked`, `repeat_block_routine_suggestion_dismissed`, `repeat_block_routine_suggestion_applied` | #531 문서 계약 추가 / 코드 구현 전 | 반복 차단 기반 추천 이유별 반응 비교. 앱 이름/package/raw history 금지 | `customEvent:suggestion_reason` |
| `time_bucket` | `repeat_block_routine_suggestion_shown`, `repeat_block_routine_suggestion_clicked`, `repeat_block_routine_suggestion_dismissed`, `repeat_block_routine_suggestion_applied` | #531 문서 계약 추가 / 코드 구현 전 | 추천 시간대별 반응 비교. raw timestamp 금지 | `customEvent:time_bucket` |
| `day_type` | `repeat_block_routine_suggestion_shown`, `repeat_block_routine_suggestion_clicked`, `repeat_block_routine_suggestion_dismissed`, `repeat_block_routine_suggestion_applied` | #531 문서 계약 추가 / 코드 구현 전 | weekday/weekend/daily/custom 패턴 비교 | `customEvent:day_type` |
| `category_bucket` | `repeat_block_routine_suggestion_shown`, `repeat_block_routine_suggestion_clicked`, `repeat_block_routine_suggestion_dismissed`, `repeat_block_routine_suggestion_applied` | #531 문서 계약 추가 / 코드 구현 전 | 앱 category bucket별 반응 비교. 앱 이름/package 금지 | `customEvent:category_bucket` |
| `repeat_count_bucket` | `repeat_block_routine_suggestion_shown`, `repeat_block_routine_suggestion_clicked`, `repeat_block_routine_suggestion_dismissed`, `repeat_block_routine_suggestion_applied` | #531 문서 계약 추가 / 코드 구현 전 | 반복 강도별 추천 반응 비교. raw retry count 금지 | `customEvent:repeat_count_bucket` |
| `routine_coverage_state` | `repeat_block_routine_suggestion_shown`, `repeat_block_routine_suggestion_clicked`, `repeat_block_routine_suggestion_dismissed`, `repeat_block_routine_suggestion_applied` | #531 문서 계약 추가 / 코드 구현 전 | 기존 루틴 커버리지 충돌/오노출 감지 | `customEvent:routine_coverage_state` |
...
### 2026-06-06T03:37:57Z metrics snapshot readback: 30일 합산 guardrail

- 확인 명령: `python3 /Users/uiel/.hermes/scripts/stopit_metrics_snapshot.py`
- window: `30daysAgo..yesterday`
- 확인된 30일 screen quality:
  - total `screen_view`: `38,338`
  - `(not set)+blank` `unifiedScreenName` gap: `24,645`
  - `(not set)` share: `24,645 / 38,338 = 64.3%`
  - visible top screens: `BlockScreen` `6,759`, `HomeScreen` `3,370`, `RoutineScreen` `680`, `OnboardingIntroScreen` `822`, `SplashScreen` `1,009`, `MenuScreen` `401`, `LockScreen` `195`
- latest observed production version adoption smoke:
  - `appVersion=1.7.7` activeUsers: `145`
  - total activeUsers: `767`
  - latest-version active share: `145 / 767 = 18.9%` → `docs/VERSION_ADOPTION_METRICS_GATE.md` 기준 `주의`
...
| `(not set)+빈 값` 비율 | `78.1%` | `TODO` | 2026-05-29 baseline은 PR #296의 `SplashScreen` / `BlockedAppsScreen` / `EmergencyUnlockSettingsScreen` 및 PR #318의 dev/debug `DevToolScreen` 보강 전 기준선. 2026-06-03 14일 live smoke는 `13,780 / 22,584 = 61.0%`였고, 2026-06-06 30일 cron snapshot은 `(not set)+blank` `24,645 / 38,338 = 64.3%`였다. 두 값 모두 PR #296/#318이 `origin/main`/`v1.7.7`에 없고 최신 `1.7.7` active share도 `145 / 767 = 18.9%`라 post-fix 성과가 아니라 중간 smoke/주의 상태로만 기록한다. PR #296/#318 포함 버전 배포 후 14일 창에서 재측정. `DevToolScreen`은 dev/debug 내부 진단 route라 production screen 품질 분모와 분리 |
