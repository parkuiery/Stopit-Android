...
2026-05-28 live 점검 메모 기준:

- `customUser:routines_count`만 metadata에서 확인됨
- `customEvent:*` 차원/지표는 아직 보이지 않음
- 최근 14일 `screen_view` 총량 `12,458`
- `(not set)` `9,390` + 빈 `unifiedScreenName` `689` = `10,079 / 12,458 = 80.9%`
...
### 2026-05-28 live queryability smoke 결과
...
| 항목 | baseline (2026-05-28 기준) | +14일 | 해석 |
| --- | --- | --- | --- |
| `screen_view` 총량 | `12,458` | `TODO` | |
| `(not set)` `unifiedScreenName` | `9,390` | `TODO` | |
| 빈 `unifiedScreenName` | `689` | `TODO` | |
| `(not set)+빈 값` 비율 | `80.9%` | `TODO` | |
