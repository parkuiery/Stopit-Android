...
| `duration_selection_type` | `goal_lock_created` | #417 문서 계약 추가 / 구현 전 | 목표 잠금 구현·배포 전후로 GA4 Admin 등록 후 metadata 확인 | `customEvent:duration_selection_type` |
| `lock_mode` | `goal_lock_created`, `goal_lock_completed`, `goal_lock_ended_early`, `goal_lock_updated` | #417 문서 계약 추가 / 구현 전 | 동일 | `customEvent:lock_mode` |
| `selected_app_count_bucket` | `goal_lock_created` | #417 문서 계약 추가 / 구현 전 | 동일 | `customEvent:selected_app_count_bucket` |
| `goal_name_type` | `goal_lock_created` | #417 문서 계약 추가 / 구현 전 | 목표 이름 원문을 보내지 않고 preset/custom enum만 확인 | `customEvent:goal_name_type` |
| `duration_days_bucket` | `goal_lock_completed` | #417 문서 계약 추가 / 구현 전 | 동일 | `customEvent:duration_days_bucket` |
| `elapsed_days_bucket` | `goal_lock_ended_early` | #417 문서 계약 추가 / 구현 전 | 동일 | `customEvent:elapsed_days_bucket` |
...
### 2026-06-03T10:21:55Z metrics snapshot readback: 30일 합산 guardrail

- 확인 명령: `python3 /Users/uiel/.hermes/scripts/stopit_metrics_snapshot.py`
- window: `30daysAgo..yesterday`
- 확인된 30일 screen quality:
  - total `screen_view`: `33,136`
  - `(not set)` `unifiedScreenName`: `22,593`
  - `(not set)` share: `22,593 / 33,136 = 68.2%`
  - visible top screens: `BlockScreen` `5,032`, `HomeScreen` `3,167`, `RoutineScreen` `637`, `OnboardingIntroScreen` `601`, `SplashScreen` `376`, `MenuScreen` `259`, `LockScreen` `134`
- latest observed production version adoption smoke:
  - `appVersion=1.7.7` activeUsers: `31`
  - total activeUsers: `688`
  - latest-version active share: `31 / 688 = 4.5%` → `docs/VERSION_ADOPTION_METRICS_GATE.md` 기준 `보류`

해석:

- 이 30일 합산 readback은 위 14일 `13,780 / 22,584 = 61.0%` smoke를 대체하는 같은 쿼리 창이 아니다. 다만 최신 cron snapshot에서도 `(not set)` 비중이 여전히 높고 최신 production cohort active share가 `10%` 미만이므로, #13을 post-fix 성과로 닫을 근거가 아니다.
...
| `(not set)+빈 값` 비율 | `78.1%` | `TODO` | 2026-05-29 baseline은 PR #296의 `SplashScreen` / `BlockedAppsScreen` / `EmergencyUnlockSettingsScreen` 및 PR #318의 dev/debug `DevToolScreen` 보강 전 기준선. 2026-06-03 14일 live smoke는 `13,780 / 22,584 = 61.0%`였고, 같은 날 30일 cron snapshot은 `(not set)` `22,593 / 33,136 = 68.2%`였다. 두 값 모두 PR #296/#318이 `origin/main`/`v1.7.7`에 없고 최신 `1.7.7` active share도 `31 / 688 = 4.5%`라 post-fix 성과가 아니라 중간 smoke로만 기록한다. PR #296/#318 포함 버전 배포 후 14일 창에서 재측정. `DevToolScreen`은 production 사용자 지표와 분리 |
