...
2. `adb shell appops set com.uiery.keep SCHEDULE_EXACT_ALARM deny` 후 exact alarm 계약을 **canonical focused deny baseline**으로 검증한다.
...
4. 남은 focused runtime gate를 아래 class/method 목록으로 실행한다.
...
5. 마지막으로 `adb shell appops set com.uiery.keep POST_NOTIFICATION ignore` 후 `ReceiverRuntimeIntegrationTest#routineAlarmReceiverWithoutPostNotificationsPermissionQueuesFallbackNoticeRehydratesDataStoreAndReschedulesEnabledRoutine`를 별도 실행한다.
