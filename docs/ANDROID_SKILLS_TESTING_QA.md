...
`.github/workflows/release-qa.yml`의 `Release instrumentation QA` job은 GitHub-hosted Android emulator에서 다음을 순서대로 실행한다.

1. Android testing skill 기반 focused release UI smoke (`com.uiery.keep.qa.StopitReleaseSmokeTest`)
2. `adb shell appops set com.uiery.keep SCHEDULE_EXACT_ALARM deny` 후 아래 focused deny 시나리오를 각각 재설치 + 재실행한다.
...
3. `adb shell appops set com.uiery.keep SCHEDULE_EXACT_ALARM allow` 후 `RoutineExactAlarmPermissionIntegrationTest#enablingRoutineWithExactAlarmPermissionSchedulesAlarm`, `RoutineExactAlarmPermissionIntegrationTest#enablingMultiDayRoutineWithExactAlarmPermissionSchedulesEveryRepeatDayAlarm`
4. remaining runtime gate로 `StopitReleaseSmokeTest`, `BackupRestoreRuntimeResetIntegrationTest`, focused `ReceiverRuntimeIntegrationTest` 7종, `EmergencyUnlockExpiryIntegrationTest`, `KeepMessagingServiceIntegrationTest`, `KeepAccessibilityServiceIntegrationTest`를 한 번 더 실행한다.
5. 마지막으로 `adb shell appops set com.uiery.keep POST_NOTIFICATION ignore` 후 `ReceiverRuntimeIntegrationTest#routineAlarmReceiverWithoutPostNotificationsPermissionQueuesFallbackNoticeRehydratesDataStoreAndReschedulesEnabledRoutine`를 별도 focused gate로 실행한다.
