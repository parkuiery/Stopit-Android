...
7. exact alarm deny gate 4 — routine alarm receiver
   ...
   - 목적: 루틴 alarm 재예약 경로가 권한 부재를 조용히 성공으로 남기지 않고 알림/프롬프트 경계와 다음 alarm 정리를 확인
   - 해석 경계: routine alarm이 이미 발화한 뒤 next-reschedule에서 `MissingExactAlarmPermission`이 발생하면 현재 triggered routine의 enabled/enforced 상태는 유지해야 한다. boot/package/restore-aftercare downgrade gate와 현재 발화 루틴 enforcement 유지 예외를 release evidence에서 섞지 않는다.
...
9. release remaining connected Android suite
   ...
   - 목적: release smoke, backup/restore runtime reset, accessibility permission resume, Room migration, routine start notification tap intent와 실제 notification-shade row tap UX, receiver 단일·multi-day 재수화/재예약, manifest contract, emergency unlock expiry, FCM token wiring, AccessibilityService cross-app block safety를 한 묶음으로 검증
...
정리하면 release candidate runtime baseline은 `focused UI smoke -> exact alarm default(MODE_DEFAULT) -> exact alarm deny(8개, multi-day 포함) -> exact alarm allow/cancel/permission-change restore(5개) -> remaining connected suite -> notification-denied receiver gate -> notification-denied emergency-unlock gate -> notification-channel-disabled gate` 순서다. Android CI focused runtime smoke는 별도 PR gate이므로 release/hotfix 증거에는 `.github/workflows/release-qa.yml`의 Release instrumentation QA 목록을 기준으로 기록한다. exact alarm/notification host ADB appops 전환은 target app 프로세스를 죽일 수 있으므로, 이 민감 receiver selector는 테스트 내부에서 전환하지 않는다. 대신 workflow/수동 명령에서 `adb shell appops set ...` → focused instrumentation 실행 순서를 유지하고, 테스트 내부 assertion은 host 상태가 잘못됐을 때 제품 회귀와 setup drift를 구분하는 진단 메시지로 남긴다. Android 12L 이하 legacy 설정 왕복과 `settings_opened` 기반 notification onboarding 검증은 historical / out of scope이며, minSdk를 다시 낮출 때만 현재 검증 대상으로 복원한다.
...
런타임 스위트는 `scripts/runtime-gate.sh`로 개발자 기기에서 돌므로 CI 진단 artifact가 없다. 실패 triage는 로컬 산출물에서 한다. 먼저 `app/build/reports/androidTests` report를 보고, 다음으로 `app/build/outputs/androidTest-results` raw result, `app/build/reports/problems` Gradle problems report, 마지막으로 `runtime-diagnostics/**`의 `logcat`, `dumpsys alarm`, `dumpsys accessibility`를 확인한다.

Android CI `Fast verification`의 scripts preflight/static policy/JVM/lint/build 실패 triage는 runtime smoke와 분리한다. `Run scripts regression tests`, `Run static policy unit tests`, `Run app unit tests`, `Run dev lint gate`, `Build prod debug APK` 중 어느 단계가 실패해도 `stopit-android-ci-fast-verification-diagnostics` artifact를 `retention-days: 7`로 업로드하며, 이 upload도 실패 원인을 가리지 않는 non-blocking 단계다. Firebase config 확인 전 scripts preflight가 실패해도 `ci-diagnostics/**`가 남아야 한다. 확인 순서는 `ci-diagnostics/scripts-regression-tests.log` → `ci-diagnostics/static-policy-unit-tests.log` → `app/build/reports/tests`의 `testDevDebugUnitTest` HTML report → `app/build/test-results` JUnit XML/raw result → `app/build/reports/lint-results-devDebug` lint report → `app/build/reports/problems` Gradle problem report → `app/build/outputs/logs` 순서다.

`app/src/test` / `app/src/androidTest` lint 제외 정책은 `docs/TEST_SOURCE_LINT_POLICY.md`를 source of truth로 본다. 현재 `app/build.gradle.kts`의 `checkTestSources = false`는 무음 면제가 아니라 명시적 정책 예외이며, Android CI/Release QA의 test 실행, runtime suite manifest, static policy tests, lint registry verification이 대체 guard다. 전체 test source lint를 켜려면 #1091 후속 QA/code package에서 warning baseline을 캡처하고, 활성화 또는 allowlist 정책을 이 문서와 workflow/static contract test에 함께 반영한다.

Release QA full JVM/lint/build, signed release build, non-production Play deploy 실패 triage artifact는 runtime smoke와 분리한다. Android Release QA의 Full release QA는 `stopit-release-qa-build-diagnostics`, Android Release Build는 `stopit-release-build-diagnostics`, non-production Android Play Deploy는 `stopit-play-deploy-release-diagnostics` artifact를 `retention-days: 7`로 업로드한다. 이 upload는 `if-no-files-found: ignore`와 `continue-on-error: true`를 쓰는 non-blocking 진단 단계이며, Release QA static policy preflight가 실패하면 먼저 `release-qa-diagnostics/static-policy-unit-tests.log`를 보고, 그 다음 `app/build/reports`의 prodRelease lint/test report, `app/build/test-results`, `app/build/reports/problems` Gradle problems report, `app/build/outputs/logs`, 마지막으로 `app/build/outputs/mapping/prodRelease`를 확인한다. Production promotion은 새 AAB를 빌드하지 않으므로 이 release diagnostics artifact 대상이 아니다.
