...
`app/src/test` / `app/src/androidTest` lint 제외 정책은 `docs/TEST_SOURCE_LINT_POLICY.md`를 source of truth로 본다. 현재 `app/build.gradle.kts`의 기본 `checkTestSources = false`는 무음 면제가 아니라 명시적 정책 예외이며, Android CI/Release QA의 test 실행, runtime suite manifest, static policy tests, lint registry verification이 대체 guard다. 전체 test source lint를 켜려면 #1091 후속 QA/code package에서 `./gradlew --console=plain :app:lintDevDebug -Pstopit.lint.checkTestSources=true` opt-in baseline을 캡처하고, 활성화 또는 allowlist 정책을 이 문서와 workflow/static contract test에 함께 반영한다.
