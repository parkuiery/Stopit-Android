...
AndroidX compileSdk / AGP boundary guard(#1008/#1051)도 같은 patch/minor policy에 포함됩니다. Stopit은 현재 `compileSdk 35` / AGP 8.x이므로, PR #989처럼 `androidx.navigationevent:*:1.0.0`이 `compileSdk 36+`를 요구하거나 `androidx.core:core:1.19.0`이 `compileSdk 37+` 및 `AGP 9.1.0 or higher`를 요구하는 batch는 app-code regression으로 디버깅하지 않습니다. PR #1042에서 `androidx.core:core-ktx 1.18.0`, `androidx.lifecycle:* 2.10.x`, `androidx.activity:activity-compose 1.12.4`, `androidx.compose:compose-bom 2025.11.x`도 같은 compileSdk/AGP-bound 실패로 확인됐으므로 `.github/dependabot.yml`은 `androidx.core:core-ktx [1.18,)`, `androidx.navigation:navigation-compose [2.9,)`, `androidx.lifecycle:* 2.10+`, `androidx.activity:activity-compose [1.12,)`, `androidx.compose:compose-bom [2025.11,)`를 `android-gradle-androidx-ui-runtime-patch-minor` lane 안에서 hold하고, 이 축은 compileSdk/AGP/Gradle/Kotlin/KSP/Compose 호환성을 함께 검증하는 별도 Android toolchain lane으로 승격합니다. PR #1043의 `com.google.devtools.ksp 2.3.9`와 PR #1045의 `org.jetbrains.kotlinx:kotlinx-serialization-json 1.11.x`도 Kotlin metadata/API 2.3.x를 요구하므로, Stopit이 Kotlin 2.1.x에 머무는 동안 각각 `com.google.devtools.ksp [2.3,)`, `org.jetbrains.kotlinx:kotlinx-serialization-json [1.11,)`로 hold하고 별도 Kotlin/toolchain lane에서 검증합니다.
