...
| --- | --- | --- | --- |
| CI | `.github/workflows/android-ci.yml` | PR to `develop`/`main`, push to `develop`/`main`, manual | Fast verification은 `:core:kds:*`와 `:app:testDevDebugUnitTest`, `:app:lintDevDebug`, `:app:assembleProdDebug`를 단일 Gradle 호출로 실행하고, `scripts.tests.test_android_manifest_contract`로 manifest/backup static policy를, `scripts/verify_lint_registry.py`로 devDebug HTML lint report의 navigation common/compose/runtime registry와 핵심 issue id를 강제 확인한다. focused runtime smoke는 `scripts/runtime-gate.sh`로 로컬에서 실행되고 CI의 `Runtime evidence` job이 `.runtime-evidence.json`의 digest를 대조한다. No signed release, no Play upload. |
...
| Governance | `version-guard.yml` | PR to `main` | main-target PR이 `release/*` / `hotfix/*`에서 왔는지, Play-safe versionCode인지 검사한다. |
...
- Dependabot PR은 repository Firebase secret을 사용할 수 없는 보안 경계가 있을 수 있다. `Fast verification`은 Firebase config availability를 먼저 확인하고, actor가 `dependabot[bot]`이며 `GOOGLE_SERVICES_JSON_DEV` / `GOOGLE_SERVICES_JSON`가 비어 있으면 dummy dev/prod `google-services.json`을 생성해 Gradle app tasks(`:app:testDevDebugUnitTest`, `:app:lintDevDebug`, `:app:assembleProdDebug`)를 계속 실행한다. `Runtime evidence`는 Dependabot이 로컬 런타임 게이트를 돌릴 수 없으므로 같은 경계를 유지해 `Dependabot Firebase secret boundary` summary와 함께 neutral-deferred 처리한다. 의존성 범프도 `gradle/libs.versions.toml`을 통해 runtime digest를 움직일 수 있으므로, 런타임 계약에 닿는 Dependabot PR은 머지 전에 사람이 `scripts/runtime-gate.sh`를 돌려 증거를 갱신하거나 `workflow_dispatch`로 emulator `Runtime smoke gate`를 실행한다. 내부 브랜치 PR에서는 stale/누락 증거를 계속 hard fail로 유지한다.
...
브랜치 이름은 **관례이고 CI 게이트가 아니다.** 전용 Branch Hygiene workflow는 PR마다 job 하나를 쓰면서 명명 규칙만 강제했기에 폐기했다. 실제로 동작을 좌우하는 규칙 하나 — main 대상 PR은 `release/*` 또는 `hotfix/*`에서 와야 한다 — 는 `Version Guard`가 모든 main 대상 PR에서 독립적으로 hard fail시킨다. `scripts/branch-start.sh`를 쓰면 관례에 맞는 이름이 자동으로 만들어진다.

Automation lane 브랜치인 `automation/*`는 PR head prefix가 아니라 **로컬 lane/worktree 안정 브랜치 전용(local lane stable branch)**이다. 예를 들어 docs lane은 `automation/stopit-docs-lane`에서 최신 `origin/develop`을 따라가다가, reviewable 작업을 만들 때는 `docs/issue-629-branch-hygiene-policy`처럼 `docs/issue-...` 또는 workflow/운영 변경이면 `ci/issue-...` 브랜치를 새로 만든다. `automation/*`를 PR head로 열지 않는다. 이제 CI가 막아주지 않으므로 lane 운영에서 지켜야 하는 규칙이다.
...
# 3. PR에서 Version Guard, Android CI, Android Release QA, Android Release Build 통과 확인
