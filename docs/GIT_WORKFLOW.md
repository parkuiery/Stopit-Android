...
- Ops CI `Docs/runbook contract tests` includes `scripts.tests.test_dependabot_policy_contract` and `scripts.tests.test_kotlin_compiler_options_contract`; docs-only / workflow-contract PRs must keep Dependabot Kotlin hold wording aligned with the actual `compilerOptions` DSL state.
