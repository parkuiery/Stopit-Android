...
긴급해제 설정 화면은 사용자가 Stopit의 차단 강도와 안전장치를 직접 조절하는 고신뢰 표면이다. 현재 `EmergencyUnlockSettingsViewModel`은 화면 진입 `screen_view(EmergencyUnlockSettingsScreen)`만 기록하고, 사용자가 실제로 바꾸는 정책(`enabled`, 일일 횟수, 허용 duration, reason required, daily/manual refill, manual reset)은 이벤트로 남기지 않는다.

이 문서는 #694의 docs-lane source of truth다. 목표는 긴급해제 설정 adoption과 마찰을 **privacy-safe enum/bucket**으로만 해석할 수 있게 code-lane 계약, GA4 등록 경계, QA evidence를 고정하는 것이다. 현재는 문서 계약 단계이며 Android wiring 전이다. 이 PR은 문서/운영 계약을 정리하며 Android 계측 구현은 후속 code-lane 경계다. 따라서 #694 PR body는 코드 구현·GA4 Admin·release/readback 전까지 `Refs #694`를 사용한다.
...
- 현재 상태: `EmergencyUnlockSettingsViewModel.init`에서 `analytics.logScreenView(KeepAnalyticsScreen.EMERGENCY_UNLOCK_SETTINGS)`만 기록한다.
...
- code-lane 구현 전 live 0건은 수요 없음으로 해석하지 않는다. live event 0건은 수요 없음이 아니라 **미계측**이다.
