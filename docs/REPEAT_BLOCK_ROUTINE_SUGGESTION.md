...
Issue: #531
상태: **code-lane policy + analytics foothold + 루틴 prefill 진입 계약 + dismiss local store + Home·LockHistory·performance_report CTA UI wiring + post_block_success BlockActivity CTA/navigation 구현 + 부분 커버 루틴 uncovered-app prefill 보강 / release·GA4 등록·수동 런타임 QA 전**
...
이 문서/PR은 repo 내부 계약을 정리하는 docs-lane work에서 시작했고, 2026-06-06 code-lane에서 로컬 후보 산출 policy(`RepeatBlockRoutineSuggestionPolicy`)와 `repeat_block_routine_suggestion_*` analytics adapter 계약까지 전진했다. PR #899(`e083495d`)는 5분 이내 같은 앱 재시도 2회 이상을 rapid_retry 구현 완료 상태로 고정하고, `RepeatBlockRoutineSuggestionPolicyTest.rapidRetryCandidateUsesRapidRetryReasonAndOutranksNewerNormalCandidate`로 rapid retry 후보가 더 최신 일반 후보보다 우선되는 회귀를 추가했다. PR #552는 추천 후보를 type-safe `RoutineRoute`로 전달해 `RoutineBottomSheetViewModel`이 시간대/요일/대상 앱 후보를 사전 입력하고, 사용자가 이름을 직접 확인·수정한 뒤 저장할 때 `repeat_block_routine_suggestion_applied`를 남기는 prefill 진입 계약을 추가했다. PR #555는 같은 추천을 닫았을 때 `time_bucket/day_type/category_bucket/dismissedAt`만 로컬 DataStore에 저장·복원하는 `RepeatBlockRoutineSuggestionStore`를 추가해 7일 재노출 제한 policy input을 실제 persist 가능한 형태로 고정했다. PR #561(`42b271f7`)은 Home과 LockHistory CTA 표면에서 추천 카드를 실제로 노출하고, apply는 루틴 prefill navigation으로, dismiss는 privacy-safe store와 dismissed analytics로 연결했다. PR #835(`8bb6592`) 보강은 Home에서 active Goal Lock card가 있는 동안 반복 차단 추천과 shown analytics를 suppress해 `현재 보호 상태 안내 > 자동 루틴 제안` 우선순위를 `HomeViewModelActivationAnalyticsTest.activeGoalLockSuppressesRepeatedBlockRoutineSuggestionAndShownAnalytics`로 고정했다. PR #843(`a37a1558`)은 active emergency unlock runtime state가 남아 있을 때 Home 반복 차단 추천과 shown analytics를 suppress하도록 `HomeViewModelActivationAnalyticsTest.activeEmergencyUnlockSuppressesRepeatedBlockRoutineSuggestionAndShownAnalytics`를 추가해 `긴급해제 진행 중 > 자동 루틴 제안` 우선순위도 고정했다. PR #887(`7202aaea`)은 LockHistory 성과 리포트 바로 아래 추천 CTA의 shown/clicked/dismissed 및 routine prefill entry surface를 `performance_report`로 분리해 performance_report 구현 표면을 repo-internal analytics 계약에 연결했다. PR #923(`19f5be8`)은 `BlockViewModel`이 `post_block_success` surface에서 최근 14일 LockHistory 기반 후보를 계산하고 shown/dismissed analytics + dismiss store를 연결했다. PR #931(`2846e6b`)은 `BlockScreen` 카드, clicked side effect, `BlockActivity → MainActivity → RoutineRoute` prefill launch를 연결해 post_block_success를 실제 구현 표면으로 승격했다. PR #944(`21bdf65e`)는 Goal Lock 차단 화면(`block_source=goal_lock`)에서 post_block_success 반복 차단 추천과 shown analytics를 suppress하도록 고정했다. PR #983(`fd7c515d`)은 기존 활성 루틴이 반복 후보 앱 전체를 이미 보호하면 추천을 숨기되, 부분 커버 상태에서는 이미 루틴이 보호하는 앱을 prefill payload에서 제외하고 uncovered 앱만 루틴 후보로 제안하도록 `RepeatBlockRoutineSuggestionPolicyTest.partiallyCoveredRoutinePrefillsOnlyUncoveredApps`로 고정했다. 이 store와 UI는 raw app name/package/list/history/timestamp를 저장·전송하지 않는다.
...
- 해당 패턴을 이미 완전히 커버하는 활성 루틴이 없는 사용자. 부분 커버 상태에서는 이미 루틴이 보호하는 앱을 prefill에서 제외하고 uncovered 앱만 제안한다.
...
반복 차단 루틴 제안은 “추천 가능한 표면이 많을수록 좋다”가 아니라, 사용자가 이미 방어 가치를 확인한 맥락에서만 한 번 부드럽게 제안해야 한다. 표면별 상태와 승격 조건은 아래처럼 분리한다.

| surface | 현재 상태 | 승격 조건 / 제한 |
| --- | --- | --- |
| `home` | 구현됨. PR #561 이후 Home CTA card가 후보 노출·apply·dismiss를 연결했고, PR #835 이후 active Goal Lock card가 있으면 추천과 shown analytics를 suppress한다. #902 이후 scheduled Goal Lock은 `isCurrentlyProtecting`이 true인 현재 window에서만 suppress한다. #1022 이후 Home CTA card layout/CTA/dismiss hierarchy는 shared `RepeatBlockRoutineSuggestionCard`가 소유하고 Home은 surface copy와 navigation/store action만 주입한다. | Home primary CTA, #455 일반 루틴 CTA, 현재 실제 보호 중인 Goal Lock card보다 강하게 보이면 실패다. |
| `lock_history` | 구현됨. PR #561 이후 LockHistory CTA surface가 같은 prefill/dismiss/store 계약을 쓴다. #1022 이후 LockHistory CTA card layout/CTA/dismiss hierarchy도 shared `RepeatBlockRoutineSuggestionCard`를 사용한다. | LockHistory 화면의 일반 기록/리스트 맥락으로 해석한다. 성과 리포트 바로 아래 CTA는 `performance_report` surface로 분리한다. |
| `post_block_success` | 구현됨. PR #923 이후 `BlockViewModel`이 후보 산출과 shown/dismissed/store를 연결했고, PR #931(`2846e6b`) 이후 `BlockScreen` 카드 + clicked side effect + `BlockActivity → MainActivity → RoutineRoute` prefill launch가 연결됐다. PR #944(`21bdf65e`) 이후 Block surface에서도 목표 잠금 차단(`block_source=goal_lock`)은 추천과 shown analytics를 suppress하도록 고정했다. #1022 이후 Block CTA card layout/CTA/dismiss hierarchy는 Home/LockHistory와 같은 shared `RepeatBlockRoutineSuggestionCard`가 소유한다. | 실제 차단 성공 직후 긍정 피드백을 해치지 않고, emergency unlock/active Goal Lock 같은 active protection 상태와 충돌하지 않으며, 노출 빈도와 dismiss 재노출 제한을 Home/LockHistory와 공유한다. release/tag/Play deploy와 device/TalkBack QA 전에는 live 0건을 UX 실패로 보지 않는다. |
| `performance_report` | performance_report 구현 표면. PR #887 이후 LockHistory 성과 리포트 직후 추천 CTA는 shown/clicked/dismissed와 Routine prefill navigation entry surface를 `performance_report`로 남긴다. | #211 공유 CTA / #407 템플릿 공유 / 광고 CTA와 slot 충돌이 없어야 하며, release/tag/Play deploy와 수동 device/TalkBack QA 전에는 live 0건을 실패로 보지 않는다. |

Docs/analytics에서 `surface=post_block_success`를 볼 수 있게 등록하더라도, 해당 Android UI wiring commit이 release/tag/Play deploy에 포함되기 전에는 live 0건을 UX 실패나 수요 없음으로 해석하지 않는다. performance_report surface도 release/tag/Play deploy와 GA4 Admin metadata 확인 전까지 live 0건을 수요 없음으로 해석하지 않는다. post-block/performance-report 표면은 Home/LockHistory와 같은 privacy-safe store/dismiss/analytics 계약을 재사용하며, 새 raw payload나 별도 추적 ID를 추가하지 않는다.
...
- 추천 저장 완료율: `routine_saved(creation_source=repeat_block_prefill)` users / `repeat_block_routine_suggestion_clicked` users, 단 `entry_surface=repeat_block_suggestion|home|lock_history|post_block_success|performance_report`로 제한
