...
| `post_block_success` | 구현됨. PR #923 이후 `BlockViewModel`이 후보 산출과 shown/dismissed/store를 연결했고, PR #931(`2846e6b`) 이후 `BlockScreen` 카드 + clicked side effect + `BlockActivity → MainActivity → RoutineRoute` prefill launch가 연결됐다. | 실제 차단 성공 직후 긍정 피드백을 해치지 않고, emergency unlock/active protection 상태와 충돌하지 않으며, 노출 빈도와 dismiss 재노출 제한을 Home/LockHistory와 공유한다. release/tag/Play deploy와 device/TalkBack QA 전에는 live 0건을 UX 실패로 보지 않는다. |
