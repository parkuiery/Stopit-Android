...
Issue: #531
상태: **code-lane policy + analytics foothold + 루틴 prefill 진입 계약 + dismiss local store + Home·LockHistory CTA UI wiring + performance_report analytics surface + post_block_success 후보 산출·shown/dismiss analytics + active Goal Lock·active emergency unlock Home suppression 구현 / post_block_success 카드 UI·click/apply navigation·release·GA4 등록·수동 런타임 QA 전**
...
| `post_block_success` | 부분 구현. PR #920 이후 `RepeatBlockRoutineSuggestionSurface.POST_BLOCK_SUCCESS`는 code/analytics enum이고, PR #923 이후 `BlockViewModel`이 차단 성공 직후 최근 14일 LockHistory 기반 후보를 산출해 shown/dismiss analytics와 bucket-only dismiss store를 남긴다. 아직 사용자에게 보이는 카드 UI, clicked/applied analytics, BlockActivity→Routine prefill navigation은 없다. | 실제 차단 성공 직후 긍정 피드백을 해치지 않고, emergency unlock/active protection 상태와 충돌하지 않으며, 노출 빈도와 dismiss 재노출 제한을 Home/LockHistory와 공유하면서 카드 UI와 click/apply navigation까지 연결될 때만 완료 표면으로 승격한다. |
...
Docs/analytics에서 `surface=post_block_success`를 볼 수 있게 등록해도, PR #923은 아직 shown/dismiss 후보 산출 경로까지만 구현한 상태다. 카드 UI, clicked/applied analytics, BlockActivity→Routine prefill navigation을 포함한 Android UI wiring PR이 `develop`에 merge되고 release/tag/Play deploy가 지나기 전에는 live 0건을 UX 실패나 수요 없음으로 해석하지 않는다. performance_report surface는 repo-internal wiring이 생겼지만 release/tag/Play deploy와 GA4 Admin metadata 확인 전까지 live 0건을 수요 없음으로 해석하지 않는다. 반대로 post-block/performance-report 표면을 구현하는 PR은 Home/LockHistory와 같은 privacy-safe store/dismiss/analytics 계약을 재사용해야 하며, 새 raw payload나 별도 추적 ID를 추가하지 않는다.
...
- [x] `post_block_success`는 차단 성공 직후 `BlockViewModel` 후보 산출, shown/dismiss analytics, bucket-only dismiss store까지 연결한다. (`BlockViewModelTest.postBlockSuccessShowsRepeatBlockSuggestionWithPrivacySafeSurface`)
- [ ] `post_block_success` 카드 UI, clicked/applied analytics, BlockActivity→Routine prefill navigation을 연결한다.
