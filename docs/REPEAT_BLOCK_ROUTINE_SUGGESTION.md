...
Issue: #531
상태: **code-lane policy + analytics foothold + 루틴 prefill 진입 계약 + dismiss local store + Home·LockHistory CTA UI wiring + active Goal Lock·active emergency unlock Home suppression 구현 / release·GA4 등록·수동 런타임 QA 전**
...
| `lock_history` | 구현됨. PR #561 이후 LockHistory CTA surface가 같은 prefill/dismiss/store 계약을 쓴다. | 성과 리포트/공유 CTA와 같은 영역에서 압박형 copy가 되면 실패다. |
| `post_block_success` | 예약 enum / 미구현 표면. 현재 차단 성공 직후 별도 CTA를 추가했다는 뜻이 아니다. | 실제 차단 성공 직후 긍정 피드백을 해치지 않고, emergency unlock/active protection 상태와 충돌하지 않으며, 노출 빈도와 dismiss 재노출 제한을 Home/LockHistory와 공유할 때만 code-lane 후보로 승격한다. |
| `performance_report` | 예약 enum / 미구현 표면. #465 LockHistory 성과 리포트 자체와 섞어 완료로 보지 않는다. | 성과 리포트가 “잘 막아낸 패턴”을 보여준 뒤 하나의 선택형 CTA로만 노출해야 하며, #211 공유 CTA / #407 템플릿 공유 / 광고 CTA와 slot 충돌이 없어야 한다. |

Docs/analytics에서 `surface=post_block_success` 또는 `surface=performance_report`를 볼 수 있게 등록해도, 해당 Android UI wiring PR이 `develop`에 merge되고 release/tag/Play deploy가 지나기 전에는 live 0건을 UX 실패나 수요 없음으로 해석하지 않는다. 반대로 post-block/performance-report 표면을 구현하는 PR은 Home/LockHistory와 같은 privacy-safe store/dismiss/analytics 계약을 재사용해야 하며, 새 raw payload나 별도 추적 ID를 추가하지 않는다.
...
- [ ] 차단 기록 없음/부족, 루틴 이미 존재, 추천 닫힘, emergency unlock 상태 QA가 있다. (policy/Home/LockHistory JVM 및 active emergency unlock suppression 완료, UI/runtime QA 전)
