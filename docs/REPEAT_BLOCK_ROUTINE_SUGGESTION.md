...
Issue: #531
상태: **code-lane policy + analytics foothold + 루틴 prefill 진입 계약 + dismiss local store + Home·LockHistory·performance_report CTA UI wiring + post_block_success BlockActivity CTA/navigation 구현 / release·GA4 등록·수동 런타임 QA 전**
...
| `post_block_success` | 구현됨. PR #923 이후 `BlockViewModel`이 후보 산출과 shown/dismissed/store를 연결했고, PR #931(`2846e6b`) 이후 `BlockScreen` 카드 + clicked side effect + `BlockActivity → MainActivity → RoutineRoute` prefill launch가 연결됐다. 이후 Block surface에서도 목표 잠금 차단(`blockSource=goal_lock`)은 추천과 shown analytics를 suppress하도록 고정했다. | 실제 차단 성공 직후 긍정 피드백을 해치지 않고, emergency unlock/active protection 상태와 충돌하지 않으며, 노출 빈도와 dismiss 재노출 제한을 Home/LockHistory와 공유한다. release/tag/Play deploy와 device/TalkBack QA 전에는 live 0건을 UX 실패로 보지 않는다. |
...
- [x] post_block_success Block surface에서도 목표 잠금 차단(`blockSource=goal_lock`)이면 반복 차단 추천과 `repeat_block_routine_suggestion_shown` analytics를 suppress한다. (`BlockViewModelTest.goalLockBlockSuppressesPostBlockSuccessRepeatBlockSuggestion`)
