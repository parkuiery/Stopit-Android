...
Issue: #531
상태: **code-lane policy + analytics foothold + 루틴 prefill 진입 계약 + dismiss local store + Home·LockHistory CTA UI wiring + performance_report analytics surface + active Goal Lock·active emergency unlock Home suppression 구현 / release·GA4 등록·수동 런타임 QA 전**
...
| `post_block_success` | post_block_success는 code constant / 미구현 UI 표면. `RepeatBlockRoutineSuggestionSurface.POST_BLOCK_SUCCESS`는 privacy-safe analytics payload에서 쓸 수 있지만, 현재 차단 성공 직후 별도 CTA를 추가했다는 뜻이 아니다. | 실제 차단 성공 직후 긍정 피드백을 해치지 않고, emergency unlock/active protection 상태와 충돌하지 않으며, 노출 빈도와 dismiss 재노출 제한을 Home/LockHistory와 공유할 때만 code-lane 후보로 승격한다. |
...
Docs/analytics에서 `surface=post_block_success`를 볼 수 있게 등록해도, 해당 Android UI wiring PR이 `develop`에 merge되고 release/tag/Play deploy가 지나기 전에는 live 0건을 UX 실패나 수요 없음으로 해석하지 않는다. performance_report surface는 repo-internal wiring이 생겼지만 release/tag/Play deploy와 GA4 Admin metadata 확인 전까지 live 0건을 수요 없음으로 해석하지 않는다. 반대로 post-block/performance-report 표면을 구현하는 PR은 Home/LockHistory와 같은 privacy-safe store/dismiss/analytics 계약을 재사용해야 하며, 새 raw payload나 별도 추적 ID를 추가하지 않는다.
