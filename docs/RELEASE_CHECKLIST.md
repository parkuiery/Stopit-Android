...
- [ ] Production promotion runs the prior internal provenance gate and must fail-fast before production secrets when the prior non-production internal release artifact or `release-provenance.json` does not match the selected tag `versionCode`, git SHA/ref, AAB `sha256`, `artifact_name`, and workflow run URL evidence.
