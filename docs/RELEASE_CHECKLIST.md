...
- [ ] Automated runtime evidence is explicit in the PR body:
  ...
  - If the branch has no fresh evidence (for example a Dependabot dependency bump), run `scripts/runtime-gate.sh` locally and commit the refreshed evidence, or dispatch `.github/workflows/android-ci.yml` with `workflow_dispatch` to run the emulator `Runtime smoke gate` job before merging.
