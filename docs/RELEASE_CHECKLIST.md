...
- [ ] If production promotion happens after the 30-day `stopit-prod-release-signed-aab` artifact retention window, the PR/run evidence records the durable fallback (`release-provenance.json` GitHub Release asset or provenance ledger) or the operator recovery path; artifact expired/missing, durable fallback missing, and provenance mismatch are classified separately.
