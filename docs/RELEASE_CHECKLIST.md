...
- [ ] If production promotion happens after the 30-day `stopit-prod-release-signed-aab` artifact retention window, `play-deploy.yml` attempts the same-tag GitHub Release `release-provenance.json` durable fallback and runs metadata-only provenance verification before production secrets; if the fallback is missing, the PR/run evidence records the operator recovery path. `artifact expired/missing`, `durable fallback missing`, and `provenance mismatch` are classified separately.
