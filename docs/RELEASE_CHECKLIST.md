...
- [ ] Non-production Play Deploy build/upload runs the same prod lint/registry gate before Google Play upload, generates `release-provenance.json`, self-verifies it before `Upload signed AAB artifact`, stores the manifest with the signed AAB artifact, uploads to Google Play through the SHA-pinned audited upload action (`r0adkll/upload-google-play@eb49699984a39f23558439581660aa6f088acfd6`), and publishes the verified `release-provenance.json` as a same-tag GitHub Release asset for durable fallback; production promotion remains a tag/versionCode promotion-only path and does not run the upload action, `:app:lintProdRelease`, build, or AAB upload.
