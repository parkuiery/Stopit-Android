...
- [ ] Any `track=production` follow-up is waiting on or has passed the GitHub Environment `production` required reviewer gate; direct GitHub dispatch and Discord-button dispatch must share this same approval boundary. `scripts/check-production-environment-approval.sh` must pass before production Play secrets are validated/decoded.
...
- [ ] Android Release QA passes on the PR:
  ...
  - `Release instrumentation QA` runs single-day and multi-day exact-alarm/runtime gates, in order, on a GitHub-hosted Android emulator:
    1. `com.uiery.keep.qa.StopitReleaseSmokeTest` on DevDebug (`:app:connectedDevDebugAndroidTest` after `:app:installDevDebug`)
    2. `com.uiery.keep.qa.StopitReleaseSmokeTest` on ProdDebug (`:app:connectedProdDebugAndroidTest` after `:app:installProdDebug`, package `com.uiery.keep`)
    3. `RoutineExactAlarmPermissionIntegrationTest#defaultExactAlarmAppOpsFollowsAlarmManagerAvailability` after `adb shell cmd appops reset com.uiery.keep.dev`
    ...
    9. `ReceiverExactAlarmPermissionIntegrationTest#routineAlarmReceiverWithExactAlarmPermissionDeniedKeepsTriggeredRoutineEnabledAndLeavesNoNextPendingIntent` after `adb shell appops set com.uiery.keep.dev SCHEDULE_EXACT_ALARM deny`
    10. `ReceiverExactAlarmPermissionIntegrationTest#routineAlarmReceiverWithExactAlarmPermissionDeniedKeepsTriggeredMultiDayRoutineEnabledAndRevokesEveryRepeatDayAlarm` after `adb shell appops set com.uiery.keep.dev SCHEDULE_EXACT_ALARM deny`
    ...
    15. `./gradlew :app:installDevDebug && ./gradlew :app:connectedDevDebugAndroidTest -Pandroid.testInstrumentationRunnerArguments.class=com.uiery.keep.notification.NotificationChannelDisabledIntegrationTest`
...
- [ ] The Release Build provenance manifest is self-verified before `Upload signed AAB artifact` with `upload-mode none` and empty Play track/status fields, so AAB checksum/size/package/version/`artifact_name`/git ref/GitHub Actions run metadata drift blocks artifact upload.
- [ ] The Release Build / non-production Play Deploy provenance manifest records AAB `sha256`, size, `versionName`, `versionCode`, `artifact_name`, git SHA/ref, track/status, GitHub Actions `run_id`, and workflow run URL, and it does not include secrets such as keystore material, service account JSON, or Firebase config.
- [ ] If Android CI Fast verification scripts/static policy/app unit/lint/build fails at `Run scripts regression tests`, `Run static policy unit tests`, `:app:testDevDebugUnitTest`, `:app:lintDevDebug`, `scripts/verify_lint_registry.py`, or `:app:assembleProdDebug`, attach `stopit-android-ci-fast-verification-diagnostics` before diagnosing from log tail alone. Triage order is `ci-diagnostics/scripts-regression-tests.log` → `ci-diagnostics/static-policy-unit-tests.log` → `app/build/reports/tests` → `app/build/test-results` → `app/build/reports/lint-results-devDebug` → `app/build/reports/problems` → `app/build/outputs/logs`; the upload uses `retention-days: 7`, `if-no-files-found: ignore`, and is non-blocking so a diagnostics upload miss/quota issue is not reported as the Fast verification failure itself.
- [ ] If Android Release QA Full release QA, Android Release Build, or non-production Play Deploy fails before/surrounding `:app:testDevDebugUnitTest`, `:app:testProdReleaseUnitTest`, `:app:lintProdRelease`, `scripts/verify_lint_registry.py`, `:app:assembleProdDebug`, `:app:bundleProdRelease`, or provenance verification, attach the short-retention diagnostics artifact first: Release QA `stopit-release-qa-build-diagnostics`, Release Build `stopit-release-build-diagnostics`, Play Deploy `stopit-play-deploy-release-diagnostics`. Triage order is Release QA static policy preflight `release-qa-diagnostics/static-policy-unit-tests.log` → `app/build/reports` → `app/build/test-results` → `app/build/reports/problems` Gradle problems report → `app/build/outputs/logs` → `app/build/outputs/mapping/prodRelease`. These uploads use `retention-days: 7`, `if-no-files-found: ignore`, and are non-blocking, so a diagnostics upload miss/quota issue is not reported as the release build failure itself.
- [ ] Non-production Play Deploy build/upload runs the same prod lint/registry gate before Google Play upload, generates `release-provenance.json`, self-verifies it before `Upload signed AAB artifact`, stores the manifest with the signed AAB artifact, and uploads to Google Play through the SHA-pinned audited upload action (`r0adkll/upload-google-play@eb49699984a39f23558439581660aa6f088acfd6`). Only successful `internal` + `release_status=completed` runs publish the verified `release-provenance.json` as the same-tag GitHub Release durable fallback; `alpha`/`beta` non-production uploads must not clobber that internal evidence asset. Production promotion remains a tag/versionCode promotion-only path and does not run the upload action, `:app:lintProdRelease`, build, or AAB upload.
- [ ] Production promotion runs the prior internal provenance gate and must fail-fast before production secrets when the prior non-production internal release artifact or `release-provenance.json` does not match the selected tag `versionCode`, git SHA/ref, AAB `sha256`, `artifact_name`, and workflow run URL evidence. The prior evidence search covers both automatic tag push artifact runs and allowed manual deploy artifact runs created by `workflow_dispatch` for the same SemVer tag/SHA, but only candidates whose manifest verifies `track=internal` + `release_status=completed` are selected; manual `alpha`/`beta`/`production` runs are skipped as `prior internal track mismatch` before fallback. Prior-artifact verification must accept a different prior `github_actions.run_id/run_attempt/run_url` from the current production-promotion run while still requiring those prior-run fields to be present and valid; same-run Release Build / non-production self-verification must continue rejecting current-run metadata drift. After Play API access is available, verify the live source track release selected by `SOURCE_TRACK=internal` and the tag `VERSION_CODE`: `sourceRelease.status` must be `completed`; `draft`/`inProgress`/`halted` should fail as a `source release status mismatch`, not as artifact expiration or provenance mismatch.
- [ ] If production promotion happens after the 30-day `stopit-prod-release-signed-aab` artifact retention window, `play-deploy.yml` attempts the same-tag GitHub Release `release-provenance.json` durable fallback and runs metadata-only provenance verification before production secrets; if the fallback is missing, the PR/run evidence records the operator recovery path. `prior internal track mismatch`, `artifact expired/missing`, `durable fallback missing`, `post-upload durable internal provenance publish failure`, and `provenance mismatch` are classified separately. If internal fallback publishing fails after `Upload to Google Play` succeeded, treat it as an evidence-publish failure rather than a Play upload failure and do not blindly re-upload the same `versionCode`; recover with a same-tag internal completed Play Deploy or verified internal fallback attachment.
...
- [ ] If app-selection, manifest permissions, Play policy declarations, or store review notes changed, `docs/QUERY_ALL_PACKAGES_POLICY.md` was reviewed; release evidence states that `QUERY_ALL_PACKAGES` remains limited to the app-selection picker, broad installed-app scans stay inside `InstalledAppRepository`, and Play Console notes use the approved Korean/English purpose + data-minimization wording.
...
- [ ] Automated runtime evidence is explicit in the PR body:
  - Android CI focused runtime smoke is a separate, lower-risk PR gate that now runs on a developer machine via `scripts/runtime-gate.sh`; cite the `.runtime-evidence.json` `runtime_digest` recorded on the branch plus the current `.github/workflows/android-ci.yml` run URL for the `Runtime evidence` job, instead of copying an old class list into the release PR.
  - If the branch has no fresh evidence (for example a Dependabot dependency bump), run `scripts/runtime-gate.sh` locally and commit the refreshed evidence before merging. No CI workflow runs an emulator, so there is no fallback path.
  - Release/hotfix PRs need the wider release sequence: `scripts/runtime-gate.sh --sequence release`. The `Release runtime evidence` job rejects narrower android-ci evidence.
  ...
  - If the runtime gate fails, triage from the local run: `app/build/reports/androidTests` → `app/build/outputs/androidTest-results`, plus `adb logcat`, `adb shell dumpsys alarm`, `adb shell dumpsys accessibility` from the device it ran against.
