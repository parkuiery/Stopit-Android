...
허용되는 값은 enum/bool/bucket뿐이다. code-lane은 `FirebaseKeepAnalyticsTest` 또는 equivalent analytics payload 테스트에서 금지 key/value가 들어가지 않음을 고정해야 한다.
...
이 docs-lane PR은 위 계약과 운영 경계를 고정한다. Android 이벤트 구현, GA4 Admin 등록, release/tag/Play deploy, 14일/30일 readback은 후속 외부/실행 경계다.
