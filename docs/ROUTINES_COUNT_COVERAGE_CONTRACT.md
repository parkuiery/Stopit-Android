...
현재 코드에서 확인된 set 위치는 `app/src/main/java/com/uiery/keep/feature/routine/RoutineViewModel.kt`의 `analytics.setUserProperty("routines_count", routines.size.toString())`다. 즉 Routine 화면/ViewModel을 거치지 않은 active user는 Room에 루틴 상태가 있어도 GA4 user property가 `(not set)`으로 남을 수 있다.
...
이 docs-lane 문서 패키지는 구현 전 계약을 고정하므로 PR body는 `Refs #479`를 사용한다. #479를 닫으려면 code-lane 구현, 테스트, release/tag/Play deploy 포함 여부, D+14/D+30 readback 또는 issue acceptance에 명시된 repo-internal 완료 조건이 충족되어야 한다.
