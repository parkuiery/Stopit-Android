...
2026-06-05 repo-observable 스냅샷에서도 같은 문제가 유지됐다. 최신 재조회(`2026-06-05T14:26:22Z`) 기준 전체 `newUsers`는 511명으로 직전 30일 349명 대비 `+46.4%`지만, `Direct` 신규 사용자가 332명으로 전체 신규의 `65.0%`까지 올라간 상태가 유지됐다. `Organic Search` 신규 사용자는 179명으로 #65 기준선 178명을 간신히 넘었지만 Direct 과다/Play Console 미확인 상태라 ASO 회복으로 승격하지 않는다. `sessions`는 4,934회로 직전 6,269회 대비 `-21.3%`이며, `Paid Search`는 활성 18명·세션 194회가 남아 있으면서 신규 사용자는 계속 0명이다. 따라서 현재 신규 유입 반등은 **ASO 회복 후보가 아니라 attribution 확인 없이는 판정 보류**로 둔다.
...
명령: `python3 /Users/uiel/.hermes/scripts/stopit_metrics_snapshot.py`
확인 시각: `2026-06-05T14:26:22Z`

| 항목 | 2026-06-05 GA4 30일 창 | 직전 30일 대비/비중 | 현재 해석 |
| --- | ---: | ---: | --- |
| 전체 `newUsers` | 511 | 직전 349 대비 +46.4% | 신규 유저 반등 폭은 더 커졌지만 channel mix 불안정은 유지 |
| 전체 `activeUsers` | 762 | 직전 601 대비 +26.8% | 활성 사용자도 반등했지만 신규 반등 원천은 아직 attribution 확인 필요 |
| 전체 `sessions` | 4,934 | 직전 6,269 대비 -21.3% | 세션은 여전히 직전 30일보다 낮아 참여 회복으로 단정 불가 |
| `Organic Search` 신규 사용자 | 179 | 179 / 511 = 35.0% | #65 기준선 178을 간신히 넘었지만 Play Console 확인 전까지 ASO 회복으로 승격 불가 |
| `Direct` 신규 사용자 | 332 | 332 / 511 = 65.0% | Direct 과다 상태가 더 커져 Play Console/external/campaign 확인이 선행 |
| `Paid Search` 신규 사용자 | 0 | 신규 비중 0% | 활성 18명·세션 194회는 신규 획득 성과로 계산하지 않음 |
...
3. 실제 Paid Search 캠페인이 집행 중인지 확인한다. 집행 중이 아니라면 최신 스냅샷의 `Paid Search` 활성 18명·세션 194회는 신규 유입 성과가 아니라 과거 사용자/재방문/분류 잔상으로 분리한다.
...
| 2026-06-05 live readback | 511 | 179 | 332 | 0 | `TODO: Play Console 수동 확인` | `TODO: Play Console 수동 확인` | `TODO: 캠페인 운영 확인` | Direct 65.0%까지 상승. Organic Search는 기준선을 간신히 넘었지만 Play Console 확인 전까지 신규 유입 반등을 ASO 효과로 표현 금지 |
...
7. #242/#65 acquisition snapshot을 고칠 때는 downstream 문서 drift를 막기 위해 `python3 -m unittest scripts.tests.test_acquisition_attribution_docs_contract -v`를 함께 실행한다.
...
| 2026-06-04 late live readback | 506 | 179 | 327 | 0 | 757 | 4,897 | `TODO` | `TODO` | `TODO` | 신규/활성 반등은 더 커졌고 Organic Search는 기준선 178을 간신히 넘었지만 Direct 64.6% 과다 때문에 Play Console/external/campaign 확인 전까지 ASO 효과 판정 보류 |
| 2026-06-05 live readback | 511 | 179 | 332 | 0 | 762 | 4,934 | `TODO` | `TODO` | `TODO` | 신규/활성 반등은 더 커졌고 Organic Search는 기준선 178을 간신히 넘었지만 Direct 65.0% 과다 때문에 Play Console/external/campaign 확인 전까지 ASO 효과 판정 보류 |
