...
2026-06-03 repo-observable 스냅샷에서도 같은 문제가 유지됐다. 최신 재조회(`2026-06-03T15:18:34Z`) 기준 전체 `newUsers`는 457명으로 직전 30일 360명 대비 `+26.9%`지만, `Direct` 신규 사용자가 281명으로 전체 신규의 `61.5%`까지 올라간 상태가 유지됐다. `Organic Search` 신규 사용자는 176명으로 #65 기준선 178명보다 아직 낮고, `sessions`는 4,744회로 직전 6,350회 대비 `-25.3%`이며, `Paid Search`는 활성 19명·세션 200회가 남아 있으면서 신규 사용자는 계속 0명이다. 따라서 현재 신규 유입 반등은 **ASO 회복 후보가 아니라 attribution 확인 없이는 판정 보류**로 둔다.
...
3. 실제 Paid Search 캠페인이 집행 중인지 확인한다. 집행 중이 아니라면 최신 스냅샷의 `Paid Search` 활성 19명·세션 200회는 신규 유입 성과가 아니라 과거 사용자/재방문/분류 잔상으로 분리한다.
