...
2026-06-03 repo-observable 스냅샷에서도 같은 문제가 유지됐다. 최신 재조회(`2026-06-03T19:07:42Z`) 기준 전체 `newUsers`는 461명으로 직전 30일 360명 대비 `+28.1%`지만, `Direct` 신규 사용자가 284명으로 전체 신규의 `61.6%`까지 올라간 상태가 유지됐다. `Organic Search` 신규 사용자는 177명으로 #65 기준선 178명보다 아직 낮고, `sessions`는 4,804회로 직전 6,350회 대비 `-24.3%`이며, `Paid Search`는 활성 19명·세션 201회가 남아 있으면서 신규 사용자는 계속 0명이다. 따라서 현재 신규 유입 반등은 **ASO 회복 후보가 아니라 attribution 확인 없이는 판정 보류**로 둔다.
...
#### 2026-06-03 third live readback

명령: `python3 /Users/uiel/.hermes/scripts/stopit_metrics_snapshot.py`
확인 시각: `2026-06-03T19:07:42Z`

| 항목 | 2026-06-03 세 번째 GA4 30일 창 | 직전 30일 대비/비중 | 현재 해석 |
| --- | ---: | ---: | --- |
| 전체 `newUsers` | 461 | 직전 360 대비 +28.1% | 신규 유저 반등 폭은 더 커졌지만 channel mix 불안정은 유지 |
| 전체 `activeUsers` | 712 | 직전 608 대비 +17.1% | 활성 사용자도 반등했지만 신규 반등 원천은 아직 attribution 확인 필요 |
| 전체 `sessions` | 4,804 | 직전 6,350 대비 -24.3% | 세션은 여전히 직전 30일보다 낮아 참여 회복으로 단정 불가 |
| `Organic Search` 신규 사용자 | 177 | 177 / 461 = 38.4% | #65 기준선 178보다 아직 낮아 ASO 회복으로 승격 불가 |
| `Direct` 신규 사용자 | 284 | 284 / 461 = 61.6% | Direct 과다 상태가 유지되어 Play Console/external/campaign 확인이 선행 |
| `Paid Search` 신규 사용자 | 0 | 신규 비중 0% | 활성 19명·세션 201회는 신규 획득 성과로 계산하지 않음 |
...
3. 실제 Paid Search 캠페인이 집행 중인지 확인한다. 집행 중이 아니라면 최신 스냅샷의 `Paid Search` 활성 19명·세션 201회는 신규 유입 성과가 아니라 과거 사용자/재방문/분류 잔상으로 분리한다.
...
| 2026-06-03 third live readback | 461 | 177 | 284 | 0 | `TODO: Play Console 수동 확인` | `TODO: Play Console 수동 확인` | `TODO: 캠페인 운영 확인` | Direct 61.6% 유지. 신규 유입 반등을 ASO 효과로 표현 금지 |
...
| 2026-06-03 third live readback | 461 | 177 | 284 | 0 | 712 | 4,804 | `TODO` | `TODO` | `TODO` | 신규/활성 반등은 더 커졌지만 Direct 61.6% 과다와 Organic Search 기준선 미회복 때문에 ASO 효과 판정 보류 |
