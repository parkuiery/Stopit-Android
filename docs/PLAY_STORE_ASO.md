...
2026-06-07 repo-observable 스냅샷에서도 같은 문제가 유지됐다. 최신 재조회(`2026-06-13T08:07:23Z`) 기준 전체 `newUsers`는 577명으로 직전 30일 272명 대비 `+112.1%`지만, `Direct` 신규 사용자가 335명으로 전체 신규의 `58.1%`로 과다 상태를 유지했다. `Organic Search` 신규 사용자는 242명으로 #65 기준선 178명을 넘었지만 Direct 과다/Play Console 미확인 상태라 ASO 회복으로 승격하지 않는다. `sessions`는 5,725회로 직전 5,704회 대비 `+0.4%`이며, `Paid Search`는 활성 19명·세션 141회가 남아 있으면서 신규 사용자는 계속 0명이다. 따라서 현재 신규 유입 반등은 **ASO 회복 후보가 아니라 attribution 확인 없이는 판정 보류**로 둔다.
...
#### 2026-06-13 live readback

명령: `python3 /Users/uiel/.hermes/scripts/stopit_metrics_snapshot.py`
확인 시각: `2026-06-13T08:07:23Z`

| 항목 | 2026-06-13 GA4 30일 창 | 직전 30일 대비/비중 | 현재 해석 |
| --- | ---: | ---: | --- |
| 전체 `newUsers` | 577 | 직전 272 대비 +112.1% | 신규 유저 반등은 유지됐지만 channel mix 불안정은 계속됨 |
| 전체 `activeUsers` | 830 | 직전 530 대비 +56.6% | 활성 사용자도 반등했지만 신규 반등 원천은 아직 attribution 확인 필요 |
| 전체 `sessions` | 5,725 | 직전 5,704 대비 +0.4% | 세션은 직전 30일과 거의 같은 수준이라 참여 회복으로 단정 불가 |
| `Organic Search` 신규 사용자 | 242 | 242 / 577 = 41.9% | #65 기준선 178을 넘었지만 Direct 과다/Play Console 미확인 전까지 ASO 회복으로 승격 불가 |
| `Direct` 신규 사용자 | 335 | 335 / 577 = 58.1% | Direct 과다 상태가 유지되어 Play Console/external/campaign 확인이 선행 |
| `Paid Search` 신규 사용자 | 0 | 신규 비중 0% | 활성 19명·세션 141회는 신규 획득 성과로 계산하지 않음 |
...
3. 실제 Paid Search 캠페인이 집행 중인지 확인한다. 집행 중이 아니라면 최신 스냅샷의 `Paid Search` 활성 19명·세션 141회는 신규 유입 성과가 아니라 과거 사용자/재방문/분류 잔상으로 분리한다.
...
| 2026-06-13 live readback | 577 | 242 | 335 | 0 | `TODO: Play Console 수동 확인` | `TODO: Play Console 수동 확인` | `TODO: 캠페인 운영 확인` | Direct 58.1% 과다 상태 유지. Organic Search는 기준선을 넘었지만 Play Console 확인 전까지 신규 유입 반등을 ASO 효과로 표현 금지 |
...
| 2026-06-13 live readback | 577 | 242 | 335 | 0 | 836 | 5,725 | `TODO` | `TODO` | `TODO` | 신규/활성 반등은 유지됐고 Organic Search는 기준선 178을 넘었지만 Direct 58.1% 과다 때문에 Play Console/external/campaign 확인 전까지 ASO 효과 판정 보류 |
