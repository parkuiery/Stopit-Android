...
2026-06-07 repo-observable 스냅샷에서도 같은 문제가 유지됐다. 최신 재조회(`2026-06-12T00:09:20Z`) 기준 전체 `newUsers`는 570명으로 직전 30일 293명 대비 `+94.5%`지만, `Direct` 신규 사용자가 334명으로 전체 신규의 `58.6%`로 과다 상태를 유지했다. `Organic Search` 신규 사용자는 236명으로 #65 기준선 178명을 넘었지만 Direct 과다/Play Console 미확인 상태라 ASO 회복으로 승격하지 않는다. `sessions`는 5,524회로 직전 5,829회 대비 `-5.2%`이며, `Paid Search`는 활성 18명·세션 144회가 남아 있으면서 신규 사용자는 계속 0명이다. 따라서 현재 신규 유입 반등은 **ASO 회복 후보가 아니라 attribution 확인 없이는 판정 보류**로 둔다.
...
#### 2026-06-12 live readback

명령: `python3 /Users/uiel/.hermes/scripts/stopit_metrics_snapshot.py`
확인 시각: `2026-06-12T00:09:20Z`

| 항목 | 2026-06-12 GA4 30일 창 | 직전 30일 대비/비중 | 현재 해석 |
| --- | ---: | ---: | --- |
| 전체 `newUsers` | 570 | 직전 293 대비 +94.5% | 신규 유저 반등은 유지됐지만 channel mix 불안정은 계속됨 |
| 전체 `activeUsers` | 829 | 직전 543 대비 +52.7% | 활성 사용자도 반등했지만 신규 반등 원천은 아직 attribution 확인 필요 |
| 전체 `sessions` | 5,524 | 직전 5,829 대비 -5.2% | 세션은 여전히 직전 30일보다 낮아 참여 회복으로 단정 불가 |
| `Organic Search` 신규 사용자 | 236 | 236 / 570 = 41.4% | #65 기준선 178을 넘었지만 Direct 과다/Play Console 미확인 전까지 ASO 회복으로 승격 불가 |
| `Direct` 신규 사용자 | 334 | 334 / 570 = 58.6% | Direct 과다 상태가 유지되어 Play Console/external/campaign 확인이 선행 |
| `Paid Search` 신규 사용자 | 0 | 신규 비중 0% | 활성 18명·세션 144회는 신규 획득 성과로 계산하지 않음 |
...
3. 실제 Paid Search 캠페인이 집행 중인지 확인한다. 집행 중이 아니라면 최신 스냅샷의 `Paid Search` 활성 18명·세션 144회는 신규 유입 성과가 아니라 과거 사용자/재방문/분류 잔상으로 분리한다.
...
| 2026-06-12 live readback | 570 | 236 | 334 | 0 | `TODO: Play Console 수동 확인` | `TODO: Play Console 수동 확인` | `TODO: 캠페인 운영 확인` | Direct 58.6% 과다 상태 유지. Organic Search는 기준선을 넘었지만 Play Console 확인 전까지 신규 유입 반등을 ASO 효과로 표현 금지 |
...
| 2026-06-12 live readback | 570 | 236 | 334 | 0 | 829 | 5,524 | `TODO` | `TODO` | `TODO` | 신규/활성 반등은 유지됐고 Organic Search는 기준선 178을 넘었지만 Direct 58.6% 과다 때문에 Play Console/external/campaign 확인 전까지 ASO 효과 판정 보류 |
