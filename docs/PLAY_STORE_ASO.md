...
2026-06-02 repo-observable 스냅샷에서도 같은 문제가 더 강해졌다. 전체 `newUsers`는 402명으로 직전 30일 362명 대비 `+11.0%`지만, `Direct` 신규 사용자가 230명으로 전체 신규의 `57.2%`까지 올라갔다. `Organic Search` 신규 사용자는 172명으로 #65 기준선 178명보다 아직 낮고, `Paid Search`는 활성 19명·세션 221회가 남아 있으면서 신규 사용자는 계속 0명이다. 따라서 현재 신규 유입 반등은 **ASO 회복 후보가 아니라 attribution 확인 없이는 판정 보류**로 둔다.
...
명령: `python3 /Users/uiel/.hermes/scripts/stopit_metrics_snapshot.py`
확인 시각: `2026-06-02T13:25:47Z`

| 항목 | 2026-06-02 GA4 30일 창 | 직전 30일 대비/비중 | 현재 해석 |
| --- | ---: | ---: | --- |
| 전체 `newUsers` | 402 | 직전 362 대비 +11.0% | 신규 유저는 반등했지만 channel mix가 ASO 효과로 보기 어려움 |
| 전체 `activeUsers` | 658 | 직전 606 대비 +8.6% | 활성 사용자도 반등했지만 세션은 여전히 약함 |
| 전체 `sessions` | 4,624 | 직전 6,372 대비 -27.4% | 유입 반등이 참여 회복으로 충분히 이어졌다고 보기 어려움 |
| `Organic Search` 신규 사용자 | 172 | 172 / 402 = 42.8% | #65 기준선 178보다 낮고 비중도 2026-06-01보다 더 낮아짐 |
| `Direct` 신규 사용자 | 230 | 230 / 402 = 57.2% | attribution 누락/외부 링크/캠페인 유입 가능성을 먼저 확인해야 함 |
| `Paid Search` 신규 사용자 | 0 | 신규 비중 0% | 활성 19명·세션 221회는 신규 획득 성과로 계산하지 않음 |
...
| 2026-06-02 중간 스냅샷 | 402 | 172 | 230 | 0 | `TODO: Play Console 수동 확인` | `TODO: Play Console 수동 확인` | `TODO: 캠페인 운영 확인` | Direct 57.2%까지 상승. 신규 유입 반등을 ASO 효과로 표현 금지 |
...
| 2026-06-02 중간 스냅샷 | 402 | 172 | 230 | 0 | 658 | 4,624 | `TODO` | `TODO` | `TODO` | 전체 신규/활성은 반등했지만 Organic Search는 기준선 178보다 낮고 Direct가 57.2%까지 커져 ASO 효과 판정 보류 |
