...
- `docs/PLAY_STORE_ASO.md`: #65용 Play Console ASO 실행 런북. 최종 copy, 스크린샷 구성, baseline, 반영 로그, 14일/30일 검증 포맷, #242 acquisition attribution gate, UTM/Install Referrer 운영 기준 포함. 현재 기준으로는 **대표님 수동 반영 완료 후 사후 복원/성과 추적 문서**다. 2026-06-11T00:10:19Z live readback에서는 전체 `newUsers`가 568명으로 반등했지만 `Direct` 신규가 333명(58.6%)으로 유지됐고 `Organic Search` 신규는 235명으로 #65 기준선 178명을 넘은 상태이며 `sessions`도 직전 30일 대비 -6.5%라, Play Console Search/Explore 확인 전에는 ASO 회복으로 표현하지 않는다. 2026-06-11 Play Console Cloud Storage `store_performance` readback은 `2026-04 -> 2026-05` store listing visitors `1,112 -> 521`(-53.1%), acquisitions `319 -> 154`(-51.7%), CVR `28.7% -> 29.6%`(+0.9p)로 5월 병목이 전환율 하락보다 방문자/노출 감소임을 보여준다. Traffic source CSV는 `Other 97.0%`로 thresholding되어 Search/Explore/external 원인 분리는 아직 외부 attribution 경계다.
...
- 2026-06-11T00:10:19Z 최신 acquisition snapshot에서는 `newUsers` 568 / 직전 298 = `+90.6%`, `activeUsers` 821 / 직전 544 = `+50.9%`였지만, `sessions`는 5,557 / 직전 5,942 = `-6.5%`이고 `Direct` 신규가 333 / 568 = `58.6%`로 과다 상태를 유지했다. `Organic Search` 신규는 235명으로 #65 기준선 178명을 넘었지만, #65/#242 판단은 신규 유저 반등보다 Play Console Search/Explore, external/campaign, Paid Search 집행 여부 확인을 우선한다.
...
- 2026-06-11T00:10:19Z metrics snapshot의 30일 합산에서도 `screen_view` `46,694` 중 `(not set)+blank` gap `28,316`(`60.6%`)가 남아 있고, 최신 관측 version `1.7.7` active share는 `267 / 821 = 32.5%`로 `충분`이다. 다만 이 30일 값은 위 14일 smoke와 같은 분모가 아니고 관련 PR package의 main/tag/Play 포함 여부가 아직 확인 경계로 남아 있으므로 추세 결론으로 합치지 않는다. 운영 판단만 고정한다: #13은 추가 screen_view 코드 PR이 아니라 PR #296/#318/#358 포함 release/tag/Play deploy 후 D+14 재측정 전까지 open 유지한다.
