...
- `docs/PLAY_STORE_ASO.md`: #65용 Play Console ASO 실행 런북. 최종 copy, 스크린샷 구성, baseline, 반영 로그, 14일/30일 검증 포맷, #242 acquisition attribution gate, UTM/Install Referrer 운영 기준 포함. 현재 기준으로는 **대표님 수동 반영 완료 후 사후 복원/성과 추적 문서**다. 2026-06-03T23:09:27Z live readback에서는 전체 `newUsers`가 464명으로 반등했지만 `Direct` 신규가 285명(61.4%)으로 유지됐고 `Organic Search` 신규는 179명으로 #65 기준선 178명을 간신히 넘은 정도이며 `sessions`도 직전 30일 대비 -23.8%라, Play Console Search/Explore 확인 전에는 ASO 회복으로 표현하지 않는다.
...
- 2026-06-03T23:09:27Z 최신 acquisition snapshot에서는 `newUsers` 464 / 직전 360 = `+28.9%`, `activeUsers` 715 / 직전 608 = `+17.6%`였지만, `sessions`는 4,837 / 직전 6,350 = `-23.8%`이고 `Direct` 신규가 285 / 464 = `61.4%`로 유지됐다. `Organic Search` 신규는 179명으로 #65 기준선 178명을 간신히 넘었지만, #65/#242 판단은 신규 유저 반등보다 Play Console Search/Explore, external/campaign, Paid Search 집행 여부 확인을 우선한다.
