...
- `docs/PARENT_MODE_MVP.md`: #471용 부모 모드 / 아이에게 폰 주기 same-device MVP 계약. PR #519/#584 이후 policy/analytics/session/Accessibility foothold를 기준선으로 두고, 보호자 PIN, 허용 앱, 시간 만료, `parent_mode_*` 이벤트, privacy-safe enum/bucket, runtime QA baseline, 원격 자녀 기기 관리 후속 gate를 정리한다. setup UI·PIN runtime·release·GA4 Admin 전까지 live `parent_mode_*` 부재는 수요 없음이 아니라 미출시/미등록 경계로 본다.
