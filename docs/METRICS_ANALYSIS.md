...
- `docs/PLAY_STORE_ASO.md`: #65용 Play Console ASO 실행 런북. 최종 copy, 스크린샷 구성, baseline, 반영 로그, 14일/30일 검증 포맷, #242 acquisition attribution gate, UTM/Install Referrer 운영 기준 포함. 현재 기준으로는 **대표님 수동 반영 완료 후 사후 복원/성과 추적 문서**다. 2026-06-02 최신 스냅샷에서는 전체 `newUsers`가 431명으로 반등했지만 `Direct` 신규가 262명(60.8%)까지 올라갔으므로, Play Console Search/Explore 확인 전에는 ASO 회복으로 표현하지 않는다.
...
- 2026-06-02 최신 acquisition snapshot에서는 `newUsers` 431 / 직전 366 = `+17.8%`, `activeUsers` 687 / 직전 613 = `+12.1%`였지만, `sessions`는 4,696 / 직전 6,401 = `-26.6%`이고 `Direct` 신규가 262 / 431 = `60.8%`까지 증가했다. #65/#242 판단은 신규 유저 반등보다 Play Console Search/Explore, external/campaign, Paid Search 집행 여부 확인을 우선한다.
