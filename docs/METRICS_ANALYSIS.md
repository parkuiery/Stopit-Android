...
- `docs/EMERGENCY_UNLOCK_STEP_ANALYTICS.md`: #779용 긴급해제 단계별 이탈·검증 실패 analytics 계약. `emergency_unlock_step_viewed` / `emergency_unlock_validation_blocked` / `emergency_unlock_cancelled`를 reason-required ON/OFF 분모와 함께 enum-only로 설계하고, custom reason 원문/app name/package/list/raw timestamp/history/duration list 금지, Android wiring·GA4 Admin·release·14일/30일 readback 경계를 분리한다.
