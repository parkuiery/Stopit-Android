...
- 2026-06-04T07:13:43Z metrics snapshot의 30일 합산에서도 `screen_view` `35,550` 중 `(not set)` `23,616`(`66.5%`)가 남아 있고, 최신 관측 version `1.7.7` active share는 `75 / 715 = 10.5%`로 `주의`다. 이 30일 값은 위 14일 smoke와 같은 분모가 아니므로 추세 결론으로 합치지 않는다. 운영 판단만 고정한다: #13은 추가 screen_view 코드 PR이 아니라 PR #296/#318 포함 release/tag/Play deploy 후 D+14 재측정 전까지 open 유지한다.
