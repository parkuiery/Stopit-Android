...
- `docs/EMERGENCY_UNLOCK_SETTINGS_ANALYTICS.md`: #694용 긴급해제 설정 변경 analytics 계약. `emergency_unlock_settings_changed` / `emergency_unlock_manual_reset_requested`를 enum/bucket만으로 설계하고, custom reason/app package/raw timestamp/manualResetAtMillis/snapshot dump 금지와 GA4 Admin/release/14일·30일 readback 경계를 고정한다. 현재는 문서 계약 단계이므로 live 0건은 수요 없음이 아니라 미계측이다.
