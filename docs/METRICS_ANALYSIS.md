...
- monetization은 광고 metadata 일부가 복구됐고 PR #293에서 `ad_banner_impression` / `ad_banner_click` / `ad_banner_revenue`로 source split 구현이 끝났다. 다만 PR #293 포함 commit은 아직 `origin/main`/`v1.7.7`에 없으므로, PR #293 포함 production 배포 후 14일 재조회 전까지 placement별 결론은 보류한다. 2026-06-11 readback에서도 `ad_banner_impression` 125건 / `ad_banner_revenue` 124건은 모두 `appVersion=1.7.5`와 `20260602..20260605`에 제한되어 source-split queryability smoke일 뿐이고, publisher surface는 `27,899` impressions 중 `(not set)`+empty `adUnitName` `13,459`건(`48.2%`)이라 placement 실험보다 매핑/source 원인 분리가 우선이다. 자세한 query template과 표는 `docs/ADMOB_MONETIZATION_RUNBOOK.md`를 따른다.
