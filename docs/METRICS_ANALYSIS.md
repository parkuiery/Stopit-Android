...
- #13 screen coverage 최신 보강: PR #755(`08d31da3`)는 Firebase `screen_view` backend payload가 canonical `screen_name`과 `screen_class`를 함께 보내도록 고정했고, PR #769(`07c7bc0a`)는 목표 잠금 생성/상세 진입에서 `GoalLockCreationScreen` / `GoalLockDetailScreen` screen_view를 기록한다. PR #913(`9d169449`)은 Parent Mode setup 진입에서 `ParentModeSetupScreen` screen_view를 기록한다. 모두 release/tag/Play deploy와 D+14 screen quality 재측정 전까지 live 성과로 승격하지 않는다.
- `docs/PLAY_STORE_ASO.md`: #65용 Play Console ASO 실행 런북. 최종 copy, 스크린샷 구성, baseline, 반영 로그, 14일/30일 검증 포맷, #242 acquisition attribution gate, UTM/Install Referrer 운영 기준 포함. 현재 기준으로는 **대표님 수동 반영 완료 후 사후 복원/성과 추적 문서**다. 2026-06-30T17:09:40Z(post-+30일) live readback에서는 전체 `newUsers`가 661명으로 직전 30일 대비 +141.2% 반등했고 `Organic Search` 신규는 409명으로 #65 기준선 178명을 크게 넘었지만 `Direct` 신규도 252명(38.1%)으로 남아 있으며 `sessions`도 직전 30일 대비 +84.0%라, Play Console Search/Explore 확인 전에는 ASO 회복으로 표현하지 않는다. 2026-06-11 Play Console Cloud Storage `store_performance` readback은 `2026-04 -> 2026-05` store listing visitors `1,112 -> 521`(-53.1%), acquisitions `319 -> 154`(-51.7%), CVR `28.7% -> 29.6%`(+0.9p)를 확인했으므로, 5월 병목은 전환율 하락보다 방문자/노출 감소로 본다. 2026-06-30 follow-up에서는 2026-06-01..16 부분월 visitors `341`, acquisitions `113`, CVR `33.1%`, visitors/day `21.3`을 확인해 전환율은 악화되지 않았고 visitor/day는 일부 회복했지만, traffic_source CSV가 여전히 `Other`로 thresholding되어 Search/Explore/external 분리는 Play Console UI/후속 attribution readback 경계다.
- `docs/INSTALL_REFERRER_ATTRIBUTION_CONTRACT.md`: #581용 Play Install Referrer / UTM attribution 계약. Direct 과다를 줄이기 위한 code-lane handoff, privacy-safe `install_referrer_attribution_checked` bucket, campaign link 기록 템플릿, GA4 Admin 등록, release/tag/Play deploy, 14일/30일 readback 경계를 분리한다.
...
- `docs/ROUTINES_COUNT_COVERAGE_CONTRACT.md`: #479용 `routines_count` user property coverage 보강 계약. `customUser:routines_count`가 metadata에 보인다는 사실과 `(not set)` activeUsers가 큰 coverage gap이라는 사실을 분리하고, PR #525 중앙 sync 구현 완료 상태와 release/D+14·D+30 readback 경계를 고정한다.
- `docs/HOME_STATUS_CTA_STRUCTURE.md`: #463용 홈 화면 상태/CTA 구조 계약. Home의 꺼짐/켜짐/타이머/목표 잠금/선택 앱 없음 상태와 단일 primary CTA 위계를 PR #500/PR #606/PR #948 landed state로 고정하되, `first_lock_configured`를 실제 차단 완료로 해석하지 않는다. 남은 경계는 실제 기기 visual/TalkBack QA, release/tag/Play deploy, GA4 Admin queryability, D+14/D+30 readback이다.
- `docs/ROUTINE_CREATION_CTA_EXPERIMENT.md`: #455용 첫 차단 성공 이후 루틴 0개 사용자 대상 루틴 생성 soft CTA 계약. `routine_creation_cta_*` 이벤트, #810 `routine_saved` 저장 완료 전환 보조 계약, privacy-safe 파라미터, Routine empty state/광고/#407 CTA 충돌 방지, 14일/30일 readback 경계를 정리한다.
- `docs/REPEAT_BLOCK_ROUTINE_SUGGESTION.md`: #531용 반복 차단 기반 자동 루틴 제안 계약. 반복 시간대·요일·앱 카테고리 bucket 기반 루틴 prefill, 기존 루틴 coverage guard, PR #561 Home/LockHistory CTA wiring, #455/#407/광고 CTA slot 충돌 방지, `repeat_block_routine_suggestion_*` analytics, #810 `routine_saved` 추천 저장 완료 전환, QA baseline을 정리한다.
- `docs/BLOCK_SCREEN_COPY_HIERARCHY.md`: #464용 차단 화면 카피/액션 위계 계약. PR #487(`8fb1911c`)로 `BlockScreen` 코칭 톤 copy, `하던 일로 돌아가기` primary CTA, emergency unlock 보조 액션/남은 횟수/disabled reason helper, locale parity, focused JVM/lint/build 검증은 `develop`에 반영됐다. PR #588(`025f9326`)로 `BlockScreenContent` 분리와 `BlockScreenContentIntegrationTest` Compose runtime baseline도 `develop`에 반영됐다. 아직 release-candidate screenshot/TalkBack QA, release/tag/Play deploy, 14일 readback 전까지는 live 성과 판단을 보류한다.
- `docs/EMERGENCY_UNLOCK_FLOW_COPY.md`: #467용 긴급해제 reason/app/duration/countdown copy·step 계약. PR #517(`572eb559`)로 짧은 reason label, helper/disabled copy, reason-required-off guardrail, 기존 reason enum compatibility, shipped locale parity와 focused JVM/lint/build 검증은 `develop`에 반영됐고, PR #575(`1a7c677`)로 reason-required ON/OFF Compose UI flow baseline도 `develop`에 반영됐으며, PR #593(`79fdee8`)로 countdown TalkBack baseline도 `develop`에 반영됐다. PR #604(`3e97f548`)로 selected reason reflection helper baseline, PR #675(`d2fab054`)로 reason/app/duration step purpose copy baseline도 `develop`에 반영됐지만 `emergency_unlock_completed.reason`은 display label/custom text가 아니라 existing enum key를 유지한다. 아직 실제 기기/screenshot/TalkBack spot-check, release/tag/Play deploy, 14일 readback 전까지는 live 성과 판단을 보류한다.
- `docs/EMERGENCY_UNLOCK_SETTINGS_ANALYTICS.md`: #694용 긴급해제 설정 변경 analytics 계약. `emergency_unlock_settings_changed` / `emergency_unlock_manual_reset_requested`를 enum/bucket만으로 Android wiring했고, PR #789 이후 동일 값 재선택·duration no-op toggle은 change 이벤트를 만들지 않아 adoption/readback 분모를 부풀리지 않는다. custom reason/app package/raw timestamp/manualResetAtMillis/snapshot dump 금지와 GA4 Admin/release/14일·30일 readback 경계를 고정한다. release/tag/Play deploy 전 live 0건은 수요 없음이 아니라 미배포/미관측이다.
- `docs/EMERGENCY_UNLOCK_STEP_ANALYTICS.md`: #779용 긴급해제 단계별 이탈·검증 실패 analytics 계약. `emergency_unlock_step_viewed` / `emergency_unlock_validation_blocked` / `emergency_unlock_cancelled`를 reason-required ON/OFF 분모와 함께 enum-only로 Android wiring했고(PR #783), PR #1086 이후 validation-blocked는 passive invalid render가 아니라 action-driven blocked attempt로만 기록한다. custom reason 원문/app name/package/list/raw timestamp/history/duration list 금지, GA4 Admin·release·14일/30일 readback 경계를 분리한다.
...
- `docs/ROUTINE_TEMPLATE_SHARE_MVP.md`: #407용 루틴 템플릿 공유 privacy-safe MVP 계약. `lockApplications`, package name, 앱 이름, raw session history를 payload/analytics에서 제외하고, `routine_template_share_*` 이벤트와 14일/30일 측정 기준을 정리한다. #778은 analytics schema 변경이 아니라 공유 payload body/label/duration의 resource-template 현지화 debt다.
- `docs/LOCK_HISTORY_PERFORMANCE_REPORT_MVP.md`: #465용 LockHistory 성과 리포트 UX 계약. 개인 성과 해석/재방문 동기를 #211 공유 루프와 분리하고, empty/low-data 카피, top apps positive framing, PR #566/#579 TalkBack contentDescription baseline, PR #805(`8f14158e`)의 선택 날짜 `selected_date` 성과 요약 분리, privacy-safe `lock_history_*` analytics bucket, 14일/30일 readback 경계를 정리한다.
...
- `docs/PARENT_MODE_MVP.md`: #471용 부모 모드 / 아이에게 폰 주기 same-device MVP 계약. PR #519/#584/#748/#870/#873/#946 이후 policy/analytics/session/Accessibility, setup/active/expired UI, 직접 분 입력, active controls fresh guardian PIN 재확인, accessibility summary foothold를 기준선으로 두고, 보호자 PIN, 허용 앱, 시간 만료, `parent_mode_*` 이벤트, privacy-safe enum/bucket, runtime QA baseline, 원격 자녀 기기 관리 후속 gate를 정리한다. release-candidate device UX spot-check·실제 기기 TalkBack 확인·release·GA4 Admin 전까지 live `parent_mode_*` 부재는 수요 없음이 아니라 미출시/미등록 경계로 본다.
- `docs/SHARED_UI_OWNERSHIP_BOUNDARY.md`: #492용 공유 UI 소유권 / feature-private import 경계. 제품 지표 문서는 아니지만, Home/Onboarding/Routine 같은 고빈도 화면 UX drift를 줄이는 engineering baseline이다. #492는 closed 상태이고 `PermissionSettingDialog` / `TimerPicker`는 app shared UI로 정리됐으므로, 후속 drift는 새 issue + static guard 갱신 대상으로 본다.
- `docs/REVIEW_PROMPT_LIFECYCLE.md`: #17용 리뷰 프롬프트 arm/drain 규칙, skip reason, Play In-App Review 한계 문서.
- `docs/REVIEW_PROMPT_POST_RELEASE_FOLLOWTHROUGH.md`: #307용 `review_prompt_shown = 0` 재측정, 버전별 lifecycle 표, Play Console 14일/30일 후행 지표 런북. 2026-06-02 기준 PR #308 launch-failure 재시도 계약과 PR #312 Home Activity unwrap 계약은 모두 develop에 merge됐고, 2026-06-02T18:06:45Z live 재조회에서 skip reason용 `customEvent:reason`은 조회 가능해졌다. 2026-06-14T06:07:23Z aggregate smoke에서도 `review_prompt_shown = 0`, `review_prompt_skipped = 45 users / 69 events`, `first_core_action_completed = 443 users`가 확인됐지만, PR #308/#312 merge commit은 여전히 `origin/main`/`v1.7.7`에 포함되지 않아 post-fix 성과나 회귀로 승격하지 않는다. 다음 판단은 PR #308/#312 포함 버전 배포 여부와 배포 후 14일 관측 창을 먼저 확인하되, failure breakdown용 `customEvent:error`는 #13 GA4 Admin 등록 경계로 유지한다.
...
- `TrackedBannerAd.kt`, `AdPlacement.kt`, `AdPlacementContractTest.kt` 기준으로 Stopit 앱 소유 배너 이벤트(`ad_banner_impression`, `ad_banner_click`, `ad_banner_revenue`)와 `screen_context`, `ad_placement`, `ad_format`, `ad_unit_id`, `ad_value_micros` 계약이 현재 코드와 일치하는지 먼저 본다. PR #461 이후 active banner call site는 `AdPlacement.toMetadata(...)`로 `ad_placement`와 `ad_unit_id`를 같은 enum source에서 생성해야 한다. PR #563 이후 `:core:kds`는 AdMob SDK runtime을 소유하지 않으므로 KDS component가 아니라 앱 monetization/analytics boundary(`TrackedBannerAd`, `AdPlacement`)를 audit한다.
...
- #531 반복 차단 기반 자동 루틴 제안은 `docs/REPEAT_BLOCK_ROUTINE_SUGGESTION.md`를 source of truth로 보고, 일반 CTA가 아니라 반복 차단 패턴이 충분한 사용자에게만 time/day/category bucket 기반 루틴 prefill을 제안한다. PR #537의 local policy + analytics adapter baseline, PR #552의 RoutineRoute/RoutineBottomSheet prefill 적용 경로, PR #555의 `RepeatBlockRoutineSuggestionStore` dismiss persistence, PR #561(`42b271f7`)의 Home/LockHistory CTA card exposure·dismiss/apply store UI wiring·locale copy parity, PR #835(`8bb6592`)의 Home active Goal Lock suppression, PR #843(`a37a1558`)의 Home active emergency unlock runtime-state suppression, PR #887(`7202aaea`)의 LockHistory 성과 리포트 직후 `performance_report` 구현 표면 분리, PR #899(`e083495d`)의 rapid_retry 후보 우선순위 회귀, PR #923(`19f5be8`)의 post_block_success 후보 산출·shown/dismissed/store는 `develop`에 반영됐다. PR #931(`2846e6b`) 이후 post_block_success card/clicked/prefill launch까지 연결됐고, PR #944(`21bdf65e`) 이후 Goal Lock-origin 차단(`block_source=goal_lock`)은 post_block_success 추천과 shown analytics를 suppress한다. PR #983(`fd7c515d`)과 PR #994(`5a5138d`) 이후 기존 활성 루틴이 반복 후보 전체를 보호하면 추천하지 않고, 부분 커버 상태에서는 이미 보호되는 앱을 prefill에서 제외해 uncovered 앱만 제안한다. #1022 이후 Block/Home/LockHistory의 추천 card layout·CTA·dismiss hierarchy는 shared `RepeatBlockRoutineSuggestionCard`가 소유한다. 구현 완료 표면은 `home`/`lock_history`/`performance_report`/`post_block_success`이며, release·GA4/readback 전에는 event 0건을 수요 없음으로 해석하지 않는다. active Goal Lock / active emergency unlock 상태이면 추천하지 않고, `repeat_block_routine_suggestion_*` 이벤트는 privacy-safe enum/bucket만 사용한다.
...
- 2026-06-01 preflight 기준 광고 custom dimensions/metrics는 GA4 metadata에 등록되어 있고, PR #293에서 앱 소유 배너 이벤트명이 SDK 자동 이벤트와 분리됐다. 이후 판단은 `docs/ADMOB_MONETIZATION_RUNBOOK.md`의 `GA4 query template: publisher surface와 Stopit 앱 custom 이벤트 분리` 및 `release boundary snapshot` 섹션을 따라 PR #293 포함 commit이 `main`/SemVer tag/Play deploy에 실제 포함된 뒤 14일 창에서 재조회한다. 2026-06-18 재조회에서도 최신 production tag `v1.7.7`과 현재 `origin/main`은 PR #293 split commit을 포함하지 않으므로, `v1.7.7` 광고 데이터는 post-split measurement로 쓰지 않는다. `ad_banner_impression`/`ad_banner_revenue`는 여전히 `appVersion=1.7.5` smoke row(`home_bottom` 113, `block_top` 12/11)로만 보이며, publisher surface의 `(not set)`+empty `adUnitName`은 `13,646 / 27,257 = 50.1%`다. active release PR #975(`release/1.7.8`) head에는 #16 관련 commits가 포함되어 있지만 PR이 `CONFLICTING/DIRTY` + no-checks 상태라 production 14일 placement 결론으로 쓰지 않는다.
...
- 이 screen 품질 수치는 PR #296의 `SplashScreen`, `BlockedAppsScreen`, `EmergencyUnlockSettingsScreen` 및 PR #318의 dev/debug `DevToolScreen` 보강 전 baseline이다. 해당 네 화면은 develop에서 명시적 `screen_view`가 추가됐고 PR #358 merge commit `6ceaecc4`가 release-boundary 해석을 고정했으므로, 동일 화면에 대한 추가 코드 작업은 `PR #296/#318/#358` 포함 버전 배포 후 14일 재측정 결과를 보고 판단한다. 단, `DevToolScreen`은 dev/debug 내부 진단 route라 production 사용자 screen 품질 분모와 분리한다.
...
- 2026-06-30T17:09:40Z post-+30일 acquisition snapshot에서는 `newUsers` 661 / 직전 274 = `+141.2%`, `activeUsers` 906 / 직전 523 = `+73.2%`, `sessions` 8,252 / 직전 4,486 = `+84.0%`로 반등이 유지됐다. `Organic Search` 신규는 409명(`61.9%`)으로 #65 기준선 178명을 크게 넘었지만 `Direct` 신규도 252 / 661 = `38.1%`로 남아 있다. 2026-06-30 Store performance follow-up도 2026-06-01..16 부분월 visitors `341`, acquisitions `113`, CVR `33.1%`로 전환율 악화보다 visitor/day 일부 회복 신호를 보였지만, traffic source가 `Other`로 thresholding되어 있다. 따라서 #65/#242 판단은 신규 유저 반등보다 Play Console Search/Explore, external/campaign, Paid Search 집행 여부 확인을 우선한다.
- monetization은 광고 metadata 일부가 복구됐고 PR #293에서 `ad_banner_impression` / `ad_banner_click` / `ad_banner_revenue`로 source split 구현이 끝났다. 다만 PR #293 포함 commit은 아직 `origin/main`/`v1.7.7`에 없으므로, PR #293 포함 production 배포 후 14일 재조회 전까지 placement별 결론은 보류한다. 2026-06-19 readback에서도 `ad_banner_impression` 125건 / `ad_banner_revenue` 124건은 모두 `appVersion=1.7.5` smoke row에 제한되어 source-split queryability smoke일 뿐이고, publisher surface는 `27,257` impressions 중 `(not set)`+empty `adUnitName` `13,646`건(`50.1%`)이라 placement 실험보다 매핑/source 원인 분리가 우선이다. `monetization_interest_shown` / `monetization_interest_clicked` row도 아직 없지만, PR #402 CTA 포함 버전의 `main`/tag/Play deploy와 GA4 Admin metadata/readback 전에는 수요 없음으로 해석하지 않는다. 자세한 query template과 표는 `docs/ADMOB_MONETIZATION_RUNBOOK.md`를 따른다.
- 2026-06-03 09:12 KST screen quality smoke에서는 최근 14일 `screen_view` `22,584`, `(not set)` `11,793`, blank `1,987`, combined `13,780 / 22,584 = 61.0%`가 확인됐다. 하지만 PR #296(`47e43784...`)과 PR #318(`8d2ee10...`)은 `origin/develop`에는 있고 `origin/main`/`v1.7.7`에는 없으므로, 이 값을 **post-fix 성과가 아니라 release boundary 전 중간 smoke**로만 본다. #13 screen quality closure는 PR #296/#318/#358 포함 release/tag/Play deploy 후 같은 쿼리 창의 **D+14 screen quality 재측정**으로 다시 판단한다.
- 2026-06-26T02:07:51Z metrics snapshot의 30일 합산에서도 `screen_view` `78,600` 중 `(not set)+blank` gap `41,100`(`52.3%`)가 남아 있고, 최신 관측 version `1.7.7` active share는 `559 / 938 = 59.6%`로 `충분`이다. 다만 이 30일 값은 위 14일 smoke와 같은 분모가 아니고 관련 PR package의 main/tag/Play 포함 여부가 아직 확인 경계로 남아 있으므로 추세 결론으로 합치지 않는다. 운영 판단만 고정한다: #13은 추가 screen_view 코드 PR이 아니라 PR #296/#318/#358 포함 release/tag/Play deploy 후 D+14 재측정 전까지 open 유지한다.
- 2026-06-12 develop에는 PR #755(`08d31da3`)의 Firebase `screen_view` backend payload 보강도 포함됐다. 이 PR은 canonical `screen_name`과 `screen_class`를 함께 보내도록 공통 adapter를 고정한 것이므로, 화면 호출 coverage package(`#296/#318/#358`)와는 별도 축이다. live screen 품질 판단은 둘 다 release/tag/Play deploy에 포함된 뒤 D+14 같은 쿼리 창에서 재측정한다.
...
- 2026-06-12 develop에는 PR #769(`07c7bc0a`)의 목표 잠금 screen coverage도 포함됐다. `GoalLockCreationScreen` / `GoalLockDetailScreen`이 canonical `screen_view`로 잡히기 전후를 비교하려면 이 commit 포함 release/tag/Play deploy와 D+14 screen quality 재측정 창을 분리한다.
- PR #913(`9d169449`)은 Parent Mode setup 진입에서 `ParentModeSetupScreen` canonical `screen_view`를 기록한다. 이 보강은 #471 Parent Mode setup 전환 해석의 선행 screen coverage이므로, 포함 release/tag/Play deploy와 D+14 screen quality 재측정 전에는 live 0건이나 `(not set)` 잔여분을 수요 없음/개선 완료로 해석하지 않는다.
