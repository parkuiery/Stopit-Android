...
- `docs/EMERGENCY_UNLOCK_STEP_ANALYTICS.md`: #779용 긴급해제 단계별 이탈·검증 실패 analytics 계약. `emergency_unlock_step_viewed` / `emergency_unlock_validation_blocked` / `emergency_unlock_cancelled`를 reason-required ON/OFF 분모와 함께 enum-only로 Android wiring했고(PR #783), custom reason 원문/app name/package/list/raw timestamp/history/duration list 금지, GA4 Admin·release·14일/30일 readback 경계를 분리한다.
...
- 2026-06-19T01:07:53Z 최신 acquisition snapshot에서는 `newUsers` 615 / 직전 217 = `+183.4%`, `activeUsers` 874 / 직전 478 = `+82.8%`였지만, `sessions`는 6,245 / 직전 5,034 = `+24.1%`이고 `Direct` 신규가 339 / 615 = `55.1%`로 과다 상태를 유지했다. `Organic Search` 신규는 276명으로 #65 기준선 178명을 넘었지만, #65/#242 판단은 신규 유저 반등보다 Play Console Search/Explore, external/campaign, Paid Search 집행 여부 확인을 우선한다.
...
- 2026-06-19T01:07:53Z metrics snapshot의 30일 합산에서도 `screen_view` `57,114` 중 `(not set)+blank` gap `31,619`(`55.4%`)가 남아 있고, 최신 관측 version `1.7.7` active share는 `395 / 874 = 45.2%`로 `충분`이다. 다만 이 30일 값은 위 14일 smoke와 같은 분모가 아니고 관련 PR package의 main/tag/Play 포함 여부가 아직 확인 경계로 남아 있으므로 추세 결론으로 합치지 않는다. 운영 판단만 고정한다: #13은 추가 screen_view 코드 PR이 아니라 PR #296/#318/#358 포함 release/tag/Play deploy 후 D+14 재측정 전까지 open 유지한다.
