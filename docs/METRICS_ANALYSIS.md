...
- `docs/PLAY_STORE_ASO.md`: #65용 Play Console ASO 실행 런북. 최종 copy, 스크린샷 구성, baseline, 반영 로그, 14일/30일 검증 포맷, #242 acquisition attribution gate, UTM/Install Referrer 운영 기준 포함. 현재 기준으로는 **대표님 수동 반영 완료 후 사후 복원/성과 추적 문서**다. 2026-06-03T15:18:34Z live readback에서는 전체 `newUsers`가 457명으로 반등했지만 `Direct` 신규가 281명(61.5%)으로 더 커졌고 `Organic Search` 신규는 176명으로 #65 기준선 178명보다 아직 낮으며 `sessions`도 직전 30일 대비 -25.3%라, Play Console Search/Explore 확인 전에는 ASO 회복으로 표현하지 않는다.
...
- `docs/ADMOB_MONETIZATION_RUNBOOK.md`: #16용 광고 단위 감사 절차, guardrail, 안전한 수익화 실험 운영 기준. PR #362 이후 `monetization_interest_*` 코드 계약은 존재하지만, 실제 CTA UI 배치·GA4 `interest_context` / `interest_surface` 등록·metadata 확인 전까지는 관심도 실험 준비 상태로 해석한다.
...
- 2026-06-03T15:18:34Z 최신 acquisition snapshot에서는 `newUsers` 457 / 직전 360 = `+26.9%`, `activeUsers` 707 / 직전 608 = `+16.3%`였지만, `sessions`는 4,744 / 직전 6,350 = `-25.3%`이고 `Direct` 신규가 281 / 457 = `61.5%`까지 커졌다. #65/#242 판단은 신규 유저 반등보다 Play Console Search/Explore, external/campaign, Paid Search 집행 여부 확인을 우선한다.
...
- 2026-06-03T10:21:55Z metrics snapshot의 30일 합산에서도 `screen_view` `33,136` 중 `(not set)` `22,593`(`68.2%`)가 남아 있고, 최신 관측 version `1.7.7` active share는 `31 / 688 = 4.5%`로 `보류`다. 이 30일 값은 위 14일 smoke와 같은 분모가 아니므로 추세 결론으로 합치지 않는다. 운영 판단만 고정한다: #13은 추가 screen_view 코드 PR이 아니라 PR #296/#318 포함 release/tag/Play deploy 후 D+14 재측정 전까지 open 유지한다.
