...
- `docs/BLOCK_SCREEN_COPY_HIERARCHY.md`: #464용 차단 화면 카피/액션 위계 계약. PR #487(`8fb1911c`)로 `BlockScreen` 코칭 톤 copy, `하던 일로 돌아가기` primary CTA, emergency unlock 보조 액션/남은 횟수/disabled reason helper, locale parity, focused JVM/lint/build 검증은 `develop`에 반영됐다. 아직 실제 기기/screenshot/TalkBack QA, release/tag/Play deploy, 14일 readback 전까지는 live 성과 판단을 보류한다.
