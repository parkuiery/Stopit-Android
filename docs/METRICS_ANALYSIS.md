...
- `docs/PLAY_STORE_ASO.md`: #65용 Play Console ASO 실행 런북. 최종 copy, 스크린샷 구성, baseline, 반영 로그, 14일/30일 검증 포맷, #242 acquisition attribution gate, UTM/Install Referrer 운영 기준 포함. 현재 기준으로는 **대표님 수동 반영 완료 후 사후 복원/성과 추적 문서**다. 2026-06-05T21:24:54Z live readback에서는 전체 `newUsers`가 520명으로 반등했지만 `Direct` 신규가 333명(64.0%)으로 유지됐고 `Organic Search` 신규는 187명으로 #65 기준선 178명을 넘은 상태이며 `sessions`도 직전 30일 대비 -21.8%라, Play Console Search/Explore 확인 전에는 ASO 회복으로 표현하지 않는다.
...
- `docs/EMERGENCY_UNLOCK_FLOW_COPY.md`: #467용 긴급해제 reason/app/duration/countdown copy·step 계약. 짧은 reason label, helper/disabled copy, reason-required-off guardrail, 기존 reason enum compatibility, privacy-safe analytics와 QA evidence 기준을 정리한다. 이 문서는 구현 완료가 아니라 code-lane UI/resource/test·locale parity·release/tag/Play deploy 후 14일 readback 전까지는 UX 계약으로만 해석한다.
...
- 2026-06-05T21:24:54Z 최신 acquisition snapshot에서는 `newUsers` 520 / 직전 346 = `+50.3%`, `activeUsers` 766 / 직전 594 = `+29.0%`였지만, `sessions`는 4,924 / 직전 6,297 = `-21.8%`이고 `Direct` 신규가 333 / 520 = `64.0%`까지 상승했다. `Organic Search` 신규는 187명으로 #65 기준선 178명을 넘었지만, #65/#242 판단은 신규 유저 반등보다 Play Console Search/Explore, external/campaign, Paid Search 집행 여부 확인을 우선한다.
...
- 2026-06-05T21:24:54Z metrics snapshot의 30일 합산에서도 `screen_view` `38,235` 중 `(not set)+blank` gap `24,600`(`64.3%`)가 남아 있고, 최신 관측 version `1.7.7` active share는 `144 / 766 = 18.8%`로 `주의`다. 이 30일 값은 위 14일 smoke와 같은 분모가 아니므로 추세 결론으로 합치지 않는다. 운영 판단만 고정한다: #13은 추가 screen_view 코드 PR이 아니라 PR #296/#318 포함 release/tag/Play deploy 후 D+14 재측정 전까지 open 유지한다.
