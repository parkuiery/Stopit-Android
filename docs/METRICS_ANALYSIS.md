...
- 2026-06-18T17:42:25Z metrics snapshot의 30일 합산에서도 `screen_view` `56,257` 중 `(not set)+blank` gap `31,162`(`55.4%`)가 남아 있고, 최신 관측 version `1.7.7` active share는 `390 / 869 = 44.9%`로 `충분`이다. 다만 이 30일 값은 위 14일 smoke와 같은 분모가 아니고 관련 PR package의 main/tag/Play 포함 여부가 아직 확인 경계로 남아 있으므로 추세 결론으로 합치지 않는다. 운영 판단만 고정한다: #13은 추가 screen_view 코드 PR이 아니라 PR #296/#318/#358 포함 release/tag/Play deploy 후 D+14 재측정 전까지 open 유지한다.
