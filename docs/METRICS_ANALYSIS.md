...
- `docs/PLAY_STORE_ASO.md`: #65용 Play Console ASO 실행 런북. 최종 copy, 스크린샷 구성, baseline, 반영 로그, 14일/30일 검증 포맷, #242 acquisition attribution gate, UTM/Install Referrer 운영 기준 포함. 현재 기준으로는 **대표님 수동 반영 완료 후 사후 복원/성과 추적 문서**다. 2026-06-03T04:10:01Z live readback에서는 전체 `newUsers`가 432명으로 반등했지만 `Direct` 신규가 263명(60.9%)으로 유지됐고 `Organic Search` 신규는 169명으로 #65 기준선 178명보다 낮으므로, Play Console Search/Explore 확인 전에는 ASO 회복으로 표현하지 않는다.
...
- 2026-06-03T04:10:01Z 최신 acquisition snapshot에서는 `newUsers` 432 / 직전 366 = `+18.0%`, `activeUsers` 688 / 직전 613 = `+12.2%`였지만, `sessions`는 4,721 / 직전 6,401 = `-26.2%`이고 `Direct` 신규가 263 / 432 = `60.9%`로 유지됐다. #65/#242 판단은 신규 유저 반등보다 Play Console Search/Explore, external/campaign, Paid Search 집행 여부 확인을 우선한다.
