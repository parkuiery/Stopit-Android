...
- `docs/PLAY_STORE_ASO.md`: #65용 Play Console ASO 실행 런북. 최종 copy, 스크린샷 구성, baseline, 반영 로그, 14일/30일 검증 포맷, #242 acquisition attribution gate, UTM/Install Referrer 운영 기준 포함. 현재 기준으로는 **대표님 수동 반영 완료 후 사후 복원/성과 추적 문서**다. 2026-06-08T02:10:51Z live readback에서는 전체 `newUsers`가 564명으로 반등했지만 `Direct` 신규가 334명(59.2%)으로 유지됐고 `Organic Search` 신규는 230명으로 #65 기준선 178명을 넘은 상태이며 `sessions`도 직전 30일 대비 -16.8%라, Play Console Search/Explore 확인 전에는 ASO 회복으로 표현하지 않는다.
...
- 2026-06-08T02:10:51Z 최신 acquisition snapshot에서는 `newUsers` 564 / 직전 357 = `+58.0%`, `activeUsers` 795 / 직전 575 = `+38.3%`였지만, `sessions`는 5,147 / 직전 6,188 = `-16.8%`이고 `Direct` 신규가 334 / 564 = `59.2%`로 과다 상태를 유지했다. `Organic Search` 신규는 230명으로 #65 기준선 178명을 넘었지만, #65/#242 판단은 신규 유저 반등보다 Play Console Search/Explore, external/campaign, Paid Search 집행 여부 확인을 우선한다.
...
- 2026-06-08T02:10:51Z metrics snapshot의 30일 합산에서도 `screen_view` `42,699` 중 `(not set)+blank` gap `26,606`(`62.3%`)가 남아 있고, 최신 관측 version `1.7.7` active share는 `205 / 795 = 25.8%`로 `주의`다. 이 30일 값은 위 14일 smoke와 같은 분모가 아니므로 추세 결론으로 합치지 않는다. 운영 판단만 고정한다: #13은 추가 screen_view 코드 PR이 아니라 PR #296/#318 포함 release/tag/Play deploy 후 D+14 재측정 전까지 open 유지한다.
