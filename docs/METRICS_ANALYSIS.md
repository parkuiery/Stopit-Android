...
- 2026-06-12T00:09:20Z 최신 acquisition snapshot에서는 `newUsers` 570 / 직전 293 = `+94.5%`, `activeUsers` 829 / 직전 543 = `+52.7%`였지만, `sessions`는 5,524 / 직전 5,829 = `-5.2%`이고 `Direct` 신규가 334 / 570 = `58.6%`로 과다 상태를 유지했다. `Organic Search` 신규는 236명으로 #65 기준선 178명을 넘었지만, #65/#242 판단은 신규 유저 반등보다 Play Console Search/Explore, external/campaign, Paid Search 집행 여부 확인을 우선한다.
...
- 2026-06-12T00:09:20Z metrics snapshot의 30일 합산에서도 `screen_view` `46,933` 중 `(not set)+blank` gap `28,255`(`60.2%`)가 남아 있고, 최신 관측 version `1.7.7` active share는 `282 / 829 = 34.0%`로 `충분`이다. 다만 이 30일 값은 위 14일 smoke와 같은 분모가 아니고 관련 PR package의 main/tag/Play 포함 여부가 아직 확인 경계로 남아 있으므로 추세 결론으로 합치지 않는다. 운영 판단만 고정한다: #13은 추가 screen_view 코드 PR이 아니라 PR #296/#318/#358 포함 release/tag/Play deploy 후 D+14 재측정 전까지 open 유지한다.
