...
- 2026-06-04T20:14:53Z 최신 acquisition snapshot에서는 `newUsers` 506 / 직전 349 = `+45.0%`, `activeUsers` 757 / 직전 601 = `+26.0%`였지만, `sessions`는 4,897 / 직전 6,269 = `-21.9%`이고 `Direct` 신규가 327 / 506 = `64.6%`까지 상승했다. `Organic Search` 신규는 179명으로 #65 기준선 178명을 간신히 넘었지만, #65/#242 판단은 신규 유저 반등보다 Play Console Search/Explore, external/campaign, Paid Search 집행 여부 확인을 우선한다.
...
- 2026-06-04T20:14:53Z metrics snapshot의 30일 합산에서도 `screen_view` `36,707` 중 `(not set)` `23,074`(`62.9%`)가 남아 있고, 최신 관측 version `1.7.7` active share는 `107 / 757 = 14.1%`로 `주의`다. 이 30일 값은 위 14일 smoke와 같은 분모가 아니므로 추세 결론으로 합치지 않는다. 운영 판단만 고정한다: #13은 추가 screen_view 코드 PR이 아니라 PR #296/#318 포함 release/tag/Play deploy 후 D+14 재측정 전까지 open 유지한다.
