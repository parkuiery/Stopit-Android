...
- `docs/PLAY_STORE_ASO.md`: #65용 Play Console ASO 실행 런북. 최종 copy, 스크린샷 구성, baseline, 반영 로그, 14일/30일 검증 포맷, #242 acquisition attribution gate, UTM/Install Referrer 운영 기준 포함. 현재 기준으로는 **대표님 수동 반영 완료 후 사후 복원/성과 추적 문서**다. 2026-06-06T03:37:57Z live readback에서는 전체 `newUsers`가 537명으로 반등했지만 `Direct` 신규가 335명(62.4%)으로 유지됐고 `Organic Search` 신규는 202명으로 #65 기준선 178명을 넘은 상태이며 `sessions`도 직전 30일 대비 -21.8%라, Play Console Search/Explore 확인 전에는 ASO 회복으로 표현하지 않는다.
...
- 2026-06-06T03:37:57Z 최신 acquisition snapshot에서는 `newUsers` 537 / 직전 384 = `+39.8%`, `activeUsers` 767 / 직전 594 = `+29.1%`였지만, `sessions`는 4,926 / 직전 6,297 = `-21.8%`이고 `Direct` 신규가 335 / 537 = `62.4%`로 과다 상태를 유지했다. `Organic Search` 신규는 202명으로 #65 기준선 178명을 넘었지만, #65/#242 판단은 신규 유저 반등보다 Play Console Search/Explore, external/campaign, Paid Search 집행 여부 확인을 우선한다.
...
- 2026-06-06T03:37:57Z metrics snapshot의 30일 합산에서도 `screen_view` `38,338` 중 `(not set)+blank` gap `24,645`(`64.3%`)가 남아 있고, 최신 관측 version `1.7.7` active share는 `145 / 767 = 18.9%`로 `주의`다. 이 30일 값은 위 14일 smoke와 같은 분모가 아니므로 추세 결론으로 합치지 않는다. 운영 판단만 고정한다: #13은 추가 screen_view 코드 PR이 아니라 PR #296/#318 포함 release/tag/Play deploy 후 D+14 재측정 전까지 open 유지한다.
