...
`routine_saved`는 #810에서 정의한 generic 루틴 저장 완료 이벤트 계약이다. PR #813(`2c4d4ae`)로 Android wiring이 `develop`에 반영됐고, PR #828(`b6d8cbd`)로 Home 보조 CTA → Routine 생성 흐름의 attribution 전달이 `entry_surface=home_secondary`, `creation_source=post_first_block_cta`로 고정됐다. 따라서 #455의 루틴 생성 전환은 CTA click 이후 `routine_saved(creation_source=post_first_block_cta)`를 직접 저장 완료 지표로 본다. 단 GA4 Admin 등록·release/tag/Play deploy·14일/30일 readback 전까지는 event 0건을 CTA 실패로 해석하지 않고, `routines_count >= 1` user property 전환과 Routine 화면/저장 흐름 evidence는 보조 지표로 유지한다.
...
- #810 `routine_saved` Android wiring은 PR #813으로 `develop`에 반영됐고, Home CTA attribution 전달은 PR #828으로 `entry_surface=home_secondary`, `creation_source=post_first_block_cta`에 연결됐다. 다만 GA4 Admin에서 `entry_surface`, `creation_source`, `selected_app_count_bucket`, `repeat_days_bucket`, `time_window_bucket`, `schedule_state`가 등록되고 metadata에서 확인되기 전에는 CTA click → 저장 완료 breakdown confidence를 낮춘다.
