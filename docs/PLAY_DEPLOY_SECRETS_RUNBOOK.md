...
| `GOOGLE_SERVICES_JSON` | `scripts/setup-play-deploy-secrets.sh --google-services <prod-json>` | Android CI, Release QA, Release Build, Play Deploy | prod Firebase config. 아래 workflow matrix 참고 |
...
해석 규칙:
- `GOOGLE_SERVICES_JSON`을 “모든 flavor 공용 secret”으로 이해하면 dev/prod `applicationId` 분리 후 Android CI / Release QA 실패 원인을 놓치기 쉽다.
...
- Release Build / Play Deploy는 prod만 요구하므로 계속 `GOOGLE_SERVICES_JSON`만 사용한다.
...
1. `docs/PLAY_DEPLOYMENT.md`
   ...
   - `GOOGLE_SERVICES_JSON` / `GOOGLE_SERVICES_JSON_DEV` 설명은 workflow별 restore matrix를 요약하고, 자세한 감사는 이 runbook을 따른다.
...
3. `docs/RELEASE_CHECKLIST.md`
   ...
   - `GOOGLE_SERVICES_JSON_DEV` dev restore와 `GOOGLE_SERVICES_JSON` prod/prod-only 차이는 runbook matrix를 따라가게 한다.
