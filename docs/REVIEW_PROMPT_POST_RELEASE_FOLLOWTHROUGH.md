...
2026-06-02T18:06:45Z GA4 snapshot (`30daysAgo..yesterday`, property `502544175`) 기준이다. 리뷰 프롬프트 lifecycle 이벤트는 이 시점의 baseline을 유지하고, ASO/Play Console 후행 판단에 쓰는 획득 채널 보조 지표는 #242/#65와 같은 2026-06-18T17:42:25Z live readback으로 최신화한다.
...
| `newUsers` | 611 | 2026-06-18T17:42:25Z acquisition readback 기준. 직전 30일 217 대비 +181.6% |
| `Organic Search` 신규 사용자 | 272 | ASO/리뷰 후행 효과 판단 보조 지표. #65 baseline 178을 넘었지만 단독 회복 근거로 승격하지 않음 |
| `Direct` 신규 사용자 | 339 | `339 / 611 = 55.5%`라 Play Console Search/Explore와 external/campaign 확인 전까지 어트리뷰션 누락/외부 유입 가능성을 분리해야 함 |
...
`stopit_metrics_snapshot.py`의 최신 30일 aggregate transport smoke에서는 `review_prompt_shown`이 `2 users`로 소량 관측되고 `review_prompt_skipped`는 `43 users / 74 events`다. 같은 snapshot의 성공 사용 신호는 `app_block_intercepted 521 users`, `lock_session_start 361 users`, `first_core_action_completed 443 users`이며, 최신 관측 version `1.7.7` active share는 `390 / 869 = 44.9%`로 `docs/VERSION_ADOPTION_METRICS_GATE.md` 기준 `충분`이다.
...
| --- | ---: | ---: | --- | ---: | ---: | --- |
| baseline | TODO | TODO | TODO | 272 | TODO | Play Console 수동 기록 필요. `Direct` 신규 339명(55.5%)이라 #242 attribution gate 적용. `Organic Search`가 #65 baseline 178을 넘었더라도 Play Console Search/Explore와 external/campaign 확인 전까지 ASO 회복으로 표현하지 않음 |
