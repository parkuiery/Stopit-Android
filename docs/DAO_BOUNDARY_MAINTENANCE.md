...
## 이번 패키지: Lock history feature boundary
...
아래 직접 DAO 의존은 아직 #520의 후속 패키지 대상이다. 이번 PR은 lock-history feature 경계만 안전하게 닫고, Receiver/Service/잠금 실행 경로는 별도 focused test와 runtime QA 범위로 다룬다.

- `BlockViewModel`: `EmergencyUnlockDao`
...
- `EmergencyUnlockCoordinator`, `ReviewEligibilityEvaluator`, `LockHistoryLedger`: service/review 경계에서 DAO를 사용하므로 별도 저장소·정책 경계 판단이 필요하다.
