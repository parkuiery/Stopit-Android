...
#875는 #520 closure audit 뒤에 새로 확인된 Home 진입 `routines_count` user property 동기화 예외였다. PR #525로 coverage foothold가 들어오면서 Home 진입 사용자도 Routine 화면 없이 Room count를 GA4 user property에 set할 수 있게 됐고, PR #881이 같은 coverage 의미를 유지하면서 DAO/Entity 결합을 RoutineRepository / RoutineModel 경계로 분리했다.

#### 허용 경계

- `RoutineRepository`가 Home `routines_count` sync의 routine read 허용 경계다.
- `RoutineCountAnalyticsSync`는 analytics boundary로 남되, `RoutineRepository.fetchAllOnce()` 또는 이미 전달받은 `RoutineModel` list에서 숫자 count만 계산한다.
- `HomeViewModel`은 Home 상태/CTA/analytics orchestration만 소유하고 Room `RoutineDao` / `RoutineEntity`를 직접 import하지 않는다.
- 이 경계는 #479의 coverage/readback 경계와 다르다. #479는 `routines_count=(not set)` 감소를 release/readback으로 검증하는 지표 이슈이고, #875는 같은 coverage foothold를 유지하면서 DAO/Entity 결합을 RoutineRepository / RoutineModel 경계로 분리한 maintenance 이슈다.

#### 회귀 방지

- `RoutineCountAnalyticsSyncTest`는 `syncFromRepository()`의 explicit `0`, 실제 routine count, 삭제 후 감소를 고정한다.
- `HomeViewModelActivationAnalyticsTest.homeInitSyncsRoutinesCountFromRoomWithoutRoutineScreenEntry`는 Routine 화면 진입 없이 Home init에서 count sync가 유지되는지 고정한다.
- `SplashViewModelRestoreSchedulingTest.splashStartupReschedulesRestoredRoomRoutineBeforeOnboardingNavigation`는 restore-aftercare 직후 restored count가 set되는 경로를 고정한다.
- `scripts.tests.test_dao_boundary_contract`는 `HomeViewModel`과 `RoutineCountAnalyticsSync`에 Room `RoutineDao` / `RoutineEntity` import가 재도입되지 않는지 검사한다.
...
future regression 발견 시에는 새 직접 DAO import를 이 문서의 허용 경계에 추가하지 말고, 해당 ViewModel/Receiver/Service를 repository/read-model 경계로 되돌리는 focused test 패키지로 처리한다. #875 경계도 allowlist로 되돌리지 말고 `scripts.tests.test_dao_boundary_contract`의 Home/RoutineCountAnalyticsSync guard를 유지한다.
