...
4. Android Settings에서 timezone을 다른 지역으로 변경한다.
5. 즉시 Splash/Menu/AccessibilityService 경로에서 잠금이 풀리거나 과도하게 연장되지 않는지 확인한다.
6. 원래 deadline의 실제 시각이 지나면 홈으로 복귀하고 `lock_history`가 완료 session으로 기록되는지 확인한다.
