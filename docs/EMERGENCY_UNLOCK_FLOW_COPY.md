...
이 문서는 #467의 source of truth다. PR #488은 문서/QA 계약을 고정했고, PR #517(`572eb559`)은 `EmergencyUnlockBottomSheetContent.kt`, `EmergencyUnlockBottomSheetState`, shipped `strings.xml` locale parity, 관련 JVM/lint/build 검증으로 reason/app/duration 단계 helper·validation copy 구현을 `develop`에 반영했다. 이제 이 문서는 code-lane 구현 전 handoff가 아니라 **develop에 반영된 구현 상태 + 남은 release/runtime/readback 경계**를 고정한다. `origin/main`/SemVer tag/Play deploy, 실제 기기/screenshot QA, 14일 readback이 끝나기 전까지 follow-up PR body는 `Refs #467`를 사용한다.
...
- 남은 UX 리스크:
  ...
  - `origin/main`/SemVer tag/Play deploy 전까지 live 지표 0건 또는 변화 없음은 post-#517 성과로 해석하면 안 된다.
...
3. release PR/tag/Play deploy에 #517 merge commit이 포함됐는지 확인한 뒤 14일 readback 창을 시작한다.
...
- docs-lane 계약과 PR #517 code-lane UI/resource/test 변경은 `develop`에 반영됐다.
