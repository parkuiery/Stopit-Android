...
- 부모 모드 / 아이에게 폰 주기는 `docs/PARENT_MODE_MVP.md`(#471)를 source of truth로 본다. MVP는 부모가 자신의 휴대폰을 아이에게 잠깐 넘기는 same-device flow이며, 보호자 PIN, 허용 앱, 시간 만료, 연장/종료 확인을 먼저 검증한다. PR #519로 policy/analytics foothold, PR #584로 session persistence와 Accessibility decision foothold가 `develop`에 반영됐고, 2026-06-09 code-lane PR로 `ParentModeSessionController`가 setup validation, session 저장, privacy-safe analytics commit, PIN 검증 후 연장/즉시 종료 commit 경계를 고정했다. 이번 code-lane PR은 Menu의 `아이에게 폰 주기` entrypoint와 setup 화면/ViewModel foothold, PIN 입력 UI와 setup CTA enablement를 추가해 현재 선택 앱을 allowed-app seed로 읽고 verified PIN일 때만 session을 시작하는 경계까지 전진시킨다. 이를 다시 구현 전 handoff로 되돌리지 않는다. 남은 경계는 active/expired 화면, device/emulator evidence, release/tag/Play deploy, GA4 Admin metadata/readback이다. 원격 자녀 기기 관리, 가족 계정, 서버 동기화는 후속 gate이고 analytics는 아이 이름·앱 이름·package·raw session history·PIN 원문 없이 enum/bucket만 사용한다.
