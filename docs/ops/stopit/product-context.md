...
- 반복 차단 기반 자동 루틴 제안(#531)은 `docs/REPEAT_BLOCK_ROUTINE_SUGGESTION.md`를 source of truth로 본다. #455가 일반 soft CTA라면 #531은 충분한 반복 차단 패턴이 관측된 뒤 시간대·요일·앱 카테고리 bucket으로 루틴 prefill을 제안하는 개인화 후보이며, 기존 활성 루틴과 겹치거나 onboarding/pre-first-lock 상태면 노출하지 않는다. 앱 이름/package/raw history/raw timestamp는 copy·analytics·registration ledger에서 금지한다.
...
- #65/#242 ASO 판단은 `docs/PLAY_STORE_ASO.md`의 attribution gate를 먼저 따른다. 2026-06-06T03:37:57Z live readback 기준 `newUsers` 537명, `Direct` 335명(62.4%), `Organic Search` 202명, `sessions` 4,926회(-21.8%)처럼 신규 유입은 반등했지만 Direct 신규 비중이 과다하고 Organic Search 신규가 #65 기준선을 넘은 상태일 때는 Play Console Search/Explore, external/campaign, UTM/Install Referrer 기록을 확인하기 전까지 ASO 회복으로 단정하지 않는다.
