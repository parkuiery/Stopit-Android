...
- #65/#242 ASO 판단은 `docs/PLAY_STORE_ASO.md`의 attribution gate를 먼저 따른다. 2026-06-04 최신 live readback처럼 `Direct` 신규 비중이 61.4% 수준으로 유지되고 `Organic Search` 신규가 #65 기준선을 간신히 넘은 정도일 때는 Play Console Search/Explore, external/campaign, UTM/Install Referrer 기록을 확인하기 전까지 ASO 회복으로 단정하지 않는다.
