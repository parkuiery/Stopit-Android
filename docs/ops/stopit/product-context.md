...
- #65/#242 ASO 판단은 `docs/PLAY_STORE_ASO.md`의 attribution gate를 먼저 따른다. 2026-06-06T22:17:11Z live readback 기준 `newUsers` 553명, `Direct` 332명(60.0%), `Organic Search` 221명, `sessions` 5,069회(-18.6%)처럼 신규 유입은 반등했지만 Direct 신규 비중이 과다하고 Organic Search 신규가 #65 기준선을 넘은 상태일 때는 Play Console Search/Explore, external/campaign, UTM/Install Referrer 기록을 확인하기 전까지 ASO 회복으로 단정하지 않는다. #581의 code/ops handoff는 `docs/INSTALL_REFERRER_ATTRIBUTION_CONTRACT.md`를 source of truth로 보고, PR #586의 parser/helper/analytics foothold와 이번 QA package의 SDK provider/첫 실행 one-shot lookup wiring은 repo 내부에 반영된 상태로 본다. 단 GA4 Admin·release/tag/Play deploy·14일/30일 readback 전에는 live Direct 개선을 주장하지 않는다.
