...
- `docs/BLOCK_SCREEN_COPY_HIERARCHY.md`: #464 차단 화면 카피/액션 위계 계약. PR #487(`8fb1911c`)로 코칭 톤, `하던 일로 돌아가기` primary CTA, emergency unlock 남은 횟수/disabled reason helper, locale parity는 `develop`에 반영됐고, 남은 기기/screenshot/TalkBack QA와 release/readback 경계를 source of truth로 고정한다.
