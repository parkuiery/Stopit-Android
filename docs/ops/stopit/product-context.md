...
- #65/#242 ASO 판단은 `docs/PLAY_STORE_ASO.md`의 attribution gate를 먼저 따른다. 2026-06-08T17:16:46Z live readback 기준 `newUsers` 570명, `Direct` 336명(58.9%), `Organic Search` 235명, `sessions` 5,176회(-15.8%)처럼 신규 유입은 반등했지만 Direct 신규 비중이 과다하고 Organic Search 신규가 #65 기준선을 넘은 상태일 때는 Play Console Search/Explore, external/campaign, UTM/Install Referrer 기록을 확인하기 전까지 ASO 회복으로 단정하지 않는다. #581의 code/ops handoff는 `docs/INSTALL_REFERRER_ATTRIBUTION_CONTRACT.md`를 source of truth로 보고, PR #586의 parser/helper/analytics foothold와 2026-06-07 QA package의 provider/first-run one-shot lookup wiring이 repo 내부에 반영됐더라도 GA4 Admin/release/readback 전에는 Direct 감소를 주장하지 않는다.
