...
- `docs/EMERGENCY_UNLOCK_FLOW_COPY.md`: #467 긴급해제 reason/app/duration/countdown copy·step 계약. PR #517(`572eb559`)로 짧은 reason label, disabled/helper copy, reason-required-off guardrail, enum compatibility, locale parity는 `develop`에 반영됐고, 남은 기기/screenshot/TalkBack QA와 release/readback 경계를 source of truth로 고정한다.
