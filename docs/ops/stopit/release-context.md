...
- CI: `.github/workflows/android-ci.yml`
  ...
  - `Fast verification` runs static policy unit tests, including `scripts.tests.test_sensitive_logging_policy`, so raw `android.util.Log` / 민감 logcat 회귀는 normal PR gate에서 차단되어야 한다.
...
- Ops CI: `.github/workflows/ops-ci.yml`
  ...
  - release-helper guardrail gate: `python3 -m unittest discover -s scripts/tests -p 'test_*.py'` plus `bash -n` on release helper shell scripts
...
- Release QA: `.github/workflows/release-qa.yml`
  ...
  - full release QA also runs the same static policy unit-test bundle, including `scripts.tests.test_sensitive_logging_policy`, before release JVM/build work.
...
- Release Build: `.github/workflows/release-build.yml`
  - scope: release/* -> main, hotfix/* -> main, or manual dispatch
  - direct push to `main` does not trigger signed release artifact generation; use release/hotfix PR gates or explicit manual dispatch instead.
...
- CD: `.github/workflows/play-deploy.yml`
  ...
  - non-production tracks build/sign/upload the signed AAB; production promotes the already-internal release matching the selected SemVer tag `versionCode`
  - 기본 track은 `internal`
...
- manual dispatch도 SemVer tag ref에서만 허용한다. branch ref는 internal/alpha/beta/production 모두 거부한다.
