...
- lane stable branch가 `automation/stopit-docs-lane` 같은 `automation/*`여도, PR head는 관례 prefix여야 한다. 브랜치 이름은 더 이상 CI가 검사하지 않으므로 lane 운영에서 지킨다. 예: 문서/런북은 `docs/issue-...`, workflow/운영 정책은 `ci/issue-...`, QA 기준/테스트는 `test/issue-...`, 앱 코드 수정은 `fix/issue-...` 또는 `feature/issue-...`.
...
공통 workflow 유지보수 기준: `Android CI`, `Ops CI`, `Play Deploy`, `Release Build`, `Release QA`, `Version Guard`는 모두 릴리즈/거버넌스 신호를 만들기 때문에 `actions/checkout` major version을 저장소 표준(v6)으로 정렬한다. 이 기준이 깨지면 코드 문제가 아니라 workflow runtime drift 때문에 Release QA나 Version Guard만 다르게 실패할 수 있으므로, `scripts.tests.test_actionlint_gate`가 회귀를 차단한다. 또한 Gradle을 실행하거나 release artifact/Play 배포 secret을 다루는 `Android CI`, `Release QA`, `Release Build`, `Play Deploy` workflow는 checkout 직후 `Gradle Wrapper`를 `gradle/actions/wrapper-validation@v6`로 검증해야 한다. 이 wrapper-validation 단계는 `Set up Gradle`, signing/Firebase/Play secret 검증·decode보다 앞에 있어야 하며, 같은 `scripts.tests.test_actionlint_gate` contract가 순서 drift를 막는다. `gradlew`, `gradlew.bat`, `gradle/wrapper/gradle-wrapper.jar`, `gradle/wrapper/gradle-wrapper.properties` 변경 PR은 wrapper-validation 증적과 contract-test green을 운영 증적으로 남긴다. Gradle distribution zip itself is pinned separately: `gradle/wrapper/gradle-wrapper.properties` must keep `distributionSha256Sum` beside `distributionUrl`, and wrapper upgrades verify the selected official checksum source such as `https://services.gradle.org/distributions/gradle-8.11.1-bin.zip.sha256`. `scripts.tests.test_gradle_wrapper_distribution_checksum` plus the Ops CI docs-contract gate guard this distribution checksum boundary.
...
- CI: `.github/workflows/android-ci.yml`
  ...
  - `Fast verification` scripts preflight/static policy/app unit/lint/build 실패 triage는 runtime smoke와 분리한다. `Run scripts regression tests`, `Run static policy unit tests`, `Run app unit tests`, `Run dev lint gate`, `Build prod debug APK` 중 어느 단계가 실패해도 `stopit-android-ci-fast-verification-diagnostics` artifact를 `retention-days: 7` / non-blocking으로 업로드한다. Firebase config 확인 전 scripts preflight가 실패해도 `ci-diagnostics/**`는 업로드 대상이며, 확인 순서는 `ci-diagnostics/scripts-regression-tests.log` → `ci-diagnostics/static-policy-unit-tests.log` → `app/build/reports/tests` HTML report → `app/build/test-results` JUnit XML/raw result → `app/build/reports/lint-results-devDebug` lint report → `app/build/reports/problems` Gradle problem report → `app/build/outputs/logs`다.
  - Android CI path gating treats `gradlew` / `gradlew.bat`, root Gradle config files, `scripts/verify_lint_registry.py`, static policy helpers (`scripts/check_compose_icon_button_accessibility.py`, `scripts/check_locale_string_parity.py`), and `.github/workflows/android-ci.yml` as **build-critical** root/script inputs, so wrapper-only or static-policy-helper-only PRs still materialize `Fast verification` instead of looking green through skipped checks.
  - Static policy helper path contract: `check_compose_icon_button_accessibility.py` must materialize `Fast verification` and run `scripts.tests.test_compose_icon_button_accessibility`; `check_locale_string_parity.py` must materialize the same gate and run `scripts.tests.test_locale_string_parity`.
  ...
  - test source lint policy: `docs/TEST_SOURCE_LINT_POLICY.md` is the source of truth for `app/build.gradle.kts` `checkTestSources = false`. This is an explicit exception, not a silent waiver. While test/androidTest source lint is excluded, Android CI must keep app unit tests, dev lint, lint-registry verification, runtime-evidence verification, and `scripts.tests.test_test_source_lint_policy` green. Enabling test source lint later requires a #1091 follow-up package with warning baseline, triage, and docs/workflow/static-contract sync.
  - Dependabot PR의 Firebase secret boundary: actor가 `dependabot[bot]`이고 `GOOGLE_SERVICES_JSON_DEV` / `GOOGLE_SERVICES_JSON` repository secret이 비어 있으면 Android CI의 `Fast verification`은 dummy dev/prod `google-services.json`을 생성해 app unit/lint/build(`:app:testDevDebugUnitTest`, `:app:lintDevDebug`, `:app:assembleProdDebug`)를 계속 materialize한다. 일반 actors와 `workflow_dispatch`에서는 secret 누락을 hard fail로 유지한다. Dependabot의 `Runtime evidence`는 로컬 런타임 게이트를 대신 돌릴 수 없으므로 같은 neutral-deferred 경계를 유지한다. 의존성 범프도 `gradle/libs.versions.toml`을 통해 runtime digest를 움직이므로, 런타임 계약에 닿는 Dependabot PR은 머지 전에 사람이 `scripts/runtime-gate.sh`로 증거를 갱신하거나 수동 `workflow_dispatch`로 emulator gate를 실행해 evidence를 채운다.
  - focused runtime smoke no longer runs on pull_request. It runs on a developer machine through `scripts/runtime-gate.sh`, which executes `ANDROID_CI_SEQUENCE` against a connected device and, only when every suite passes, writes `.runtime-evidence.json` with each suite result plus a `runtime_digest` over the runtime sources listed in `RUNTIME_DIGEST_INPUTS`. The Android CI `Runtime evidence` job recomputes that digest with no emulator, JDK or Gradle and fails on stale or missing evidence, so an app/runtime change cannot merge on an outdated local run. The `runtime_smoke` paths filter must stay a superset of `RUNTIME_DIGEST_INPUTS`, which `scripts.tests.test_runtime_evidence_contract` enforces; otherwise a digest-moving change could skip the evidence job entirely.
  - suite selectors run batched: one Gradle invocation per suite instead of one per selector, because host appops/install isolation is a suite boundary. A failed batch replays its selectors individually to name the culprit, and a batch failure that no single selector reproduces is reported as suspected cross-test interference (`--no-batch` reverts to per-selector runs).
  - No workflow runs an emulator, including `workflow_dispatch`. The aggregate diagnostic sequence still exists in `scripts/android_runtime_suites.py` and is what the local gate drives: `android_ci_focused_runtime_smoke` failure does not hide later exact-alarm / notification-denied / channel-disabled suite results, but the job still exits non-zero after the aggregate summary when any selector or before-command failed. The PR gate includes a minimal exact-alarm smoke (`android_ci_exact_alarm_default`, `android_ci_exact_alarm_denied`, `android_ci_exact_alarm_allowed`) while full single-day/multi-day exact-alarm coverage remains release-only. This Android CI PR gate is intentionally separate from `Android Release QA / Release instrumentation QA`; release/hotfix PR evidence should cite the recorded `runtime_digest` plus the Android CI `Runtime evidence` run URL for this layer and use the exact Release QA list below for main-target release readiness, including the multi-day exact-alarm/runtime gates.
...
- Ops CI: `.github/workflows/ops-ci.yml`
  ...
  - Docs/runbook contract tests gate: docs-only PR/push and release/CI/CD workflow 변경 PR (`android-ci.yml`, `release-qa.yml`, `release-build.yml`, `play-deploy.yml`, `version-guard.yml`) materialize a lightweight Python/static contract job that runs `python3 -m unittest discover -s scripts/tests -p 'test_*.py'`. This prevents docs-reading static contracts such as `scripts.tests.test_shared_ui_component_boundaries`, `scripts.tests.test_feature_domain_boundary_contract`, `scripts.tests.test_feature_domain_boundary_context_pack`, `scripts.tests.test_dao_boundary_maintenance_docs`, `scripts.tests.test_dependabot_policy_contract`, and `scripts.tests.test_kotlin_compiler_options_contract` from being omitted when a hardcoded module list drifts. The job intentionally avoids `npm ci`, Gradle, emulator, Firebase/Play/signing secrets, and Android build work. `actionlint-only green`은 workflow syntax만 증명하고, `contract-test green`이 operator docs/runbook/source-of-truth drift, Gradle wrapper distribution checksum drift, Kotlin compilerOptions DSL drift, workflow Gradle task guard drift까지 막는다. README 현재 버전 drift의 경우 `scripts.tests.test_readme_version_contract`가 `README.md` ↔ `app/build.gradle.kts` 동기화, `scripts/bump-version.sh` 업데이트 계약, `scripts/release-tag.sh` tag 직전 mismatch 중단, operator docs의 최종 확인 문구를 함께 고정한다.
...
- Release QA: `.github/workflows/release-qa.yml`
  ...
  - Release QA uses the same #1091 test source lint policy boundary as Android CI: `:app:lintProdRelease` verifies production source lint and lint-registry coverage, while test/androidTest source lint remains explicitly excluded until a warning-baseline/allowlist package changes `docs/TEST_SOURCE_LINT_POLICY.md` and `scripts.tests.test_test_source_lint_policy` together.
  - `Full release QA` diagnostics are separate from runtime smoke artifacts. Static policy preflight output is tee'd to `release-qa-diagnostics/static-policy-unit-tests.log` and uploaded inside `stopit-release-qa-build-diagnostics` with `release-qa-diagnostics/**`, `app/build/reports/**`, `app/build/reports/problems/**` Gradle problems report, `app/build/test-results/**`, `app/build/outputs/logs/**`, and `app/build/outputs/mapping/prodRelease/**`; this upload remains `if: always()`, `continue-on-error: true`, and `retention-days: 7`.
  - Release instrumentation selector source of truth is `scripts/android_runtime_suites.py`; workflows own install/host ADB appops sequencing, while the manifest owns selector fragments. #1030 기준으로 exact-alarm/notification appops 전환은 target app 프로세스를 죽일 수 있으므로 receiver tests는 테스트 내부에서 전환하지 않는다. Host setup drift는 named suite before-command와 테스트 assertion 메시지로 제품 회귀와 분리한다. Android CI PR gate is intentionally separate; use the exact Release QA suite sequence below.
  ...
  - exact alarm denied gate: `adb shell appops set com.uiery.keep.dev SCHEDULE_EXACT_ALARM deny` 후 아래 focused instrumentation을 순서대로 실행
    ...
    - `ReceiverExactAlarmPermissionIntegrationTest#routineAlarmReceiverWithExactAlarmPermissionDeniedKeepsTriggeredRoutineEnabledAndLeavesNoNextPendingIntent`
  ...
  - #609 해석 경계: routine alarm이 이미 발화한 뒤 next-reschedule에서 MissingExactAlarmPermission이 발생하면 현재 triggered routine의 enabled/enforced 상태를 유지해야 한다. release evidence에서는 boot/package/restore-aftercare downgrade gate와 현재 발화 루틴 enforcement 유지 예외를 분리해 기록한다. 예외 source of truth는 `docs/ACTIVE_ROUTINE_ENFORCEMENT_CONTRACT.md`와 `RoutineReceiverPolicyTest#applyRoutineAlarmRescheduleResultKeepsTriggeredRoutineEnabledWhenExactAlarmPermissionMissing`다.
...
- Release Build: `.github/workflows/release-build.yml`
  ...
  - app-selection package visibility, manifest permission wording, or Play review notes change 때는 `docs/QUERY_ALL_PACKAGES_POLICY.md`를 release evidence source of truth로 보고, `QUERY_ALL_PACKAGES`가 app-selection picker / `InstalledAppRepository` / `SelectableAppPolicy` 경계에만 남아 있으며 설치 앱 전체 목록을 analytics·광고·프로파일링·서버 전송·제3자 공유에 쓰지 않는다는 문안을 확인한다.
  ...
  - Release Build failures upload `stopit-release-build-diagnostics` with `app/build/reports/**`, `app/build/reports/problems/**` Gradle problems report, `app/build/test-results/**`, `app/build/outputs/logs/**`, and `app/build/outputs/mapping/prodRelease/**` before operator triage relies on log tails alone.
...
- CD: `.github/workflows/play-deploy.yml`
  ...
  - non-production Play Deploy failures upload `stopit-play-deploy-release-diagnostics` with `app/build/reports/**`, `app/build/reports/problems/**` Gradle problems report, `app/build/test-results/**`, `app/build/outputs/logs/**`, and `app/build/outputs/mapping/prodRelease/**`; production promotion is excluded because it does not build a new AAB.
  - the Play upload action pin is a release-critical provenance boundary, not a routine dependency bump. Refresh it only through a reviewed PR that updates `.github/workflows/play-deploy.yml`, `scripts.tests.test_release_provenance_workflow_contract`, and operator docs together; production promotion uses the repo-owned promotion helper and does not run this upload action. `.github/dependabot.yml` therefore ignores `r0adkll/upload-google-play` even for patch/minor updates; Dependabot noise on this action should be closed or replaced by a release-governance PR after manual provenance review.
  - production promotes the already-internal release matching the selected SemVer tag `versionCode` and does not run `:app:lintProdRelease`; production promotion uses the prior non-production manifest as the audit boundary rather than creating a new production AAB/provenance artifact. After Play API access is available, the repo-owned promotion helper must also verify the live source track release selected by `SOURCE_TRACK=internal` and `VERSION_CODE`: `sourceRelease.status` must be `completed`; `draft`/`inProgress`/`halted` source releases are a `source release status mismatch` / live Play source-track state blocker, not artifact expiration or provenance mismatch.
  ...
  - Play Deploy sets up Node 22 with `actions/setup-node@v6` before release-helper JS execution (`scripts/validate-play-rollout-inputs.js` and `scripts/promote-google-play-track.js`) so deployment uses the same Node runtime that Ops CI validates.
...
- `./gradlew :app:installDevDebug && adb shell appops set com.uiery.keep.dev SCHEDULE_EXACT_ALARM deny && ./gradlew :app:connectedDevDebugAndroidTest -Pandroid.testInstrumentationRunnerArguments.class=com.uiery.keep.receiver.ReceiverExactAlarmPermissionIntegrationTest#routineAlarmReceiverWithExactAlarmPermissionDeniedKeepsTriggeredRoutineEnabledAndLeavesNoNextPendingIntent`
- `./gradlew :app:installDevDebug && adb shell appops set com.uiery.keep.dev SCHEDULE_EXACT_ALARM deny && ./gradlew :app:connectedDevDebugAndroidTest -Pandroid.testInstrumentationRunnerArguments.class=com.uiery.keep.receiver.ReceiverExactAlarmPermissionIntegrationTest#routineAlarmReceiverWithExactAlarmPermissionDeniedKeepsTriggeredMultiDayRoutineEnabledAndRevokesEveryRepeatDayAlarm`
...
- `main` 대상 PR에서는 `Version Guard`가 항상 생성되어야 한다. 정상적인 release/hotfix PR은 `Android Release QA / Full release QA`, `Android Release QA / Release instrumentation QA`, `Android Release Build`, `Version Guard`가 모두 green이어야 merge한다.
