...
- CI: `.github/workflows/android-ci.yml`
  ...
  - pull_request / manual runs also execute focused runtime smoke for the current `.github/workflows/android-ci.yml` class list. Android CI runs this through `scripts/android_runtime_suites.py run-android-ci` aggregate diagnostic mode: `android_ci_focused_runtime_smoke` failure does not hide later notification-denied/channel-disabled suite results, but the job still exits non-zero after the aggregate summary when any selector or before-command failed. This Android CI PR gate is intentionally separate from `Android Release QA / Release instrumentation QA`; release/hotfix PR evidence should cite the Android CI run URL for this layer and use the exact Release QA list below for main-target release readiness, including the multi-day exact-alarm/runtime gates.
