...
- CD: `.github/workflows/play-deploy.yml`
  ...
  - non-production tracks run `:app:lintProdRelease` + prodRelease lint registry verification before build/sign/upload of the signed AAB; they generate/verify `release-provenance.json` before `Upload to Google Play` and upload it alongside the signed AAB artifact
