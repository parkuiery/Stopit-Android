...
- CI: `.github/workflows/android-ci.yml`
  ...
  - `Fast verification` app unit/lint/build 실패 triage는 runtime smoke와 분리한다. `Run app unit tests`, `Run dev lint gate`, `Build prod debug APK` 중 어느 단계가 실패해도 `stopit-android-ci-fast-verification-diagnostics` artifact를 `retention-days: 7` / non-blocking으로 업로드하며, 확인 순서는 `app/build/reports/tests` HTML report → `app/build/test-results` JUnit XML/raw result → `app/build/reports/lint-results-devDebug` lint report → `app/build/reports/problems` Gradle problem report → `app/build/outputs/logs`다.
