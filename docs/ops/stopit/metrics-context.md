...
- Play Store 링크를 새로 배포하거나 캠페인을 시작할 때는 가능한 한 `utm_source`, `utm_medium`, `utm_campaign`과 게시 시각을 기록하고, #65 판정표에는 GA4 채널과 Play Console Search/Explore/external source를 함께 남긴다. #581은 PR #586으로 parser/helper/analytics foothold가, 이번 QA package로 SDK provider/첫 실행 one-shot lookup wiring이 repo 내부에 반영됐지만, GA4 Admin metadata·release/tag/Play deploy·D+14/D+30 readback 전에는 Direct 감소나 ASO 회복을 주장하지 않는다.
- 2026-06-08 live readback(`2026-06-08T23:09:54Z`)에서도 전체 `newUsers`가 571명으로 직전 30일 대비 +65.0%였지만 `Direct` 신규가 336명(58.8%)으로 유지됐고 `Organic Search` 신규는 236명으로 #65 기준선 178명을 넘은 상태이며 `sessions`는 5,244회로 직전 6,149회 대비 -14.7%다. 따라서 현재 신규 유입 반등은 #242 외부 확인 전까지 ASO 회복이 아니라 attribution 판정 보류 신호로 본다.
...
- 2026-06-08T23:09:54Z metrics snapshot의 30일 합산에서는 `screen_view` `44,376` 중 `(not set)+blank` gap이 `27,369`(`61.7%`)였고, 최신 관측 version `1.7.7` active share도 `232 / 806 = 28.8%`로 `주의`다. 이 값은 위 14일 smoke와 직접 합산하지 않고, #13을 release/tag/Play deploy + D+14 재측정 전까지 닫지 않는 guardrail로만 둔다.
