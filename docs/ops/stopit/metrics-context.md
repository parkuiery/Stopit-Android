...
- 2026-06-08 live readback(`2026-06-08T17:16:46Z`)에서도 전체 `newUsers`가 570명으로 직전 30일 대비 +64.7%였지만 `Direct` 신규가 336명(58.9%)으로 유지됐고 `Organic Search` 신규는 235명으로 #65 기준선 178명을 넘은 상태이며 `sessions`는 5,176회로 직전 6,149회 대비 -15.8%다. 따라서 현재 신규 유입 반등은 #242 외부 확인 전까지 ASO 회복이 아니라 attribution 판정 보류 신호로 본다.
...
- 2026-06-08T17:16:46Z metrics snapshot의 30일 합산에서는 `screen_view` `43,622` 중 `(not set)+blank` gap이 `26,970`(`61.8%`)였고, 최신 관측 version `1.7.7` active share도 `226 / 803 = 28.1%`로 `주의`다. 이 값은 위 14일 smoke와 직접 합산하지 않고, #13을 release/tag/Play deploy + D+14 재측정 전까지 닫지 않는 guardrail로만 둔다.
