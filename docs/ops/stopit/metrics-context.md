...
- 2026-06-07 live readback(`2026-06-07T16:13:51Z`)에서도 전체 `newUsers`가 560명으로 직전 30일 대비 +56.9%였지만 `Direct` 신규가 334명(59.6%)으로 유지됐고 `Organic Search` 신규는 227명으로 #65 기준선 178명을 넘은 상태이며 `sessions`는 5,126회로 직전 6,188회 대비 -17.2%다. 따라서 현재 신규 유입 반등은 #242 외부 확인 전까지 ASO 회복이 아니라 attribution 판정 보류 신호로 본다.
...
- 2026-06-07T16:13:51Z metrics snapshot의 30일 합산에서는 `screen_view` `41,792` 중 `(not set)+blank` gap이 `26,138`(`62.5%`)였고, 최신 관측 version `1.7.7` active share도 `199 / 792 = 25.1%`로 `주의`다. 이 값은 위 14일 smoke와 직접 합산하지 않고, #13을 release/tag/Play deploy + D+14 재측정 전까지 닫지 않는 guardrail로만 둔다.
