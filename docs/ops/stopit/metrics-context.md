...
- 2026-06-09 live readback(`2026-06-09T15:25:30Z`)에서도 전체 `newUsers`가 571명으로 직전 30일 대비 +65.5%였지만 `Direct` 신규가 336명(58.8%)으로 유지됐고 `Organic Search` 신규는 236명으로 #65 기준선 178명을 넘은 상태이며 `sessions`는 5,298회로 직전 6,088회 대비 -13.0%다. 따라서 현재 신규 유입 반등은 #242 외부 확인 전까지 ASO 회복이 아니라 attribution 판정 보류 신호로 본다.
...
- 2026-06-09T15:25:30Z metrics snapshot의 30일 합산에서는 `screen_view` `44,814` 중 `(not set)+blank` gap이 `27,468`(`61.3%`)였고, 최신 관측 version `1.7.7` active share도 `246 / 805 = 30.6%`로 `충분`이다. 다만 관련 PR package의 main/tag/Play 포함 여부와 D+14 같은 쿼리 창 재측정 전까지 #13을 닫지 않는 guardrail로만 둔다.
