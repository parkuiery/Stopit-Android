...
- 2026-06-06 live readback(`2026-06-06T19:06:27Z`)에서도 전체 `newUsers`가 551명으로 직전 30일 대비 +49.3%였지만 `Direct` 신규가 332명(60.3%)으로 유지됐고 `Organic Search` 신규는 219명으로 #65 기준선 178명을 넘은 상태이며 `sessions`는 5,047회로 직전 6,226회 대비 -18.9%다. 따라서 현재 신규 유입 반등은 #242 외부 확인 전까지 ASO 회복이 아니라 attribution 판정 보류 신호로 본다.
...
- 긴급해제 플로우 copy/step 개선(#467)은 `docs/EMERGENCY_UNLOCK_FLOW_COPY.md`를 source of truth로 본다. PR #517(`572eb559`)로 reason/app/duration helper·validation copy, reason-required-off helper, shipped locale parity, focused JVM/lint/build 검증은 `develop`에 반영됐다. 기존 `emergency_unlock_used` / `emergency_unlock_completed(reason, duration_minutes, remaining_unlocks)` 의미와 reason enum key를 유지하는 것이 1차 경계이며, 새 이벤트를 요구하지 않는다. Reason-required-off 사용자는 reason distribution confidence를 낮추는 별도 분모로 보고, custom reason 원문·앱 이름/package/raw selected app list/raw history는 payload나 query 축으로 쓰지 않는다. 실제 기기/screenshot/TalkBack QA, `origin/main`/SemVer tag/Play deploy, 14일 관측 전에는 긴급해제율/완료율 변화를 copy 성과로 단정하지 않는다.
...
- 2026-06-06T19:06:27Z metrics snapshot의 30일 합산에서는 `screen_view` `39,810` 중 `(not set)+blank` gap이 `25,260`(`63.5%`)였고, 최신 관측 version `1.7.7` active share도 `178 / 781 = 22.8%`로 `주의`다. 이 값은 위 14일 smoke와 직접 합산하지 않고, #13을 release/tag/Play deploy + D+14 재측정 전까지 닫지 않는 guardrail로만 둔다.
