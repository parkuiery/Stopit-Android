...
- 차단 화면 카피/액션 위계(#464)는 `docs/BLOCK_SCREEN_COPY_HIERARCHY.md`를 source of truth로 본다. PR #487(`8fb1911c`)로 `BlockScreen` 코칭 톤, `하던 일로 돌아가기` primary CTA, `EmergencyUnlockActionUiPolicy` helper, locale parity, focused JVM/lint/build 검증은 `develop`에 반영됐다. 기존 `app_block_intercepted`, `first_core_action_completed`, `core_action_completed`, `emergency_unlock_used`, `emergency_unlock_completed`, `ad_banner_*` 의미를 깨지 않는 것이 1차 경계이며, 새 이벤트를 요구하지 않는다. 별도 실험 이벤트를 추가하더라도 앱 이름/package/raw history/raw timestamp는 금지하고, 실제 기기/screenshot/TalkBack QA·release/tag/Play deploy·14일 관측 전에는 차단 화면 copy 성과를 live 지표로 단정하지 않는다.
