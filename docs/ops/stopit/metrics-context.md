...
- 이번 code-lane follow-through로 `ParentModeSetupScreen` canonical `screen_view` coverage도 추가됐다. 이는 #471 Parent Mode setup 전환 해석의 선행 screen coverage이므로, 부모 모드 live 결론은 해당 coverage 포함 release/tag/Play deploy와 D+14 screen quality 재측정 뒤에만 올린다.
