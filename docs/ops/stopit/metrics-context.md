...
- Play Store 링크를 새로 배포하거나 캠페인을 시작할 때는 가능한 한 `utm_source`, `utm_medium`, `utm_campaign`과 게시 시각을 기록하고, #65 판정표에는 GA4 채널과 Play Console Search/Explore/external source를 함께 남긴다. #581은 PR #586으로 parser/helper/analytics foothold가 `develop`에 반영됐지만, SDK provider/첫 실행 lookup·GA4 Admin metadata·release/tag/Play deploy·D+14/D+30 readback 전에는 Direct 감소나 ASO 회복을 주장하지 않는다.
