...
- 2026-06-06 live readback(`2026-06-06T08:33:25Z`)에서도 전체 `newUsers`가 537명으로 직전 30일 대비 +39.8%였지만 `Direct` 신규가 335명(62.4%)으로 유지됐고 `Organic Search` 신규는 202명으로 #65 기준선 178명을 넘은 상태이며 `sessions`는 4,951회로 직전 6,297회 대비 -21.4%다. 따라서 현재 신규 유입 반등은 #242 외부 확인 전까지 ASO 회복이 아니라 attribution 판정 보류 신호로 본다.
...
- LockHistory 성과 리포트(#465)는 `docs/LOCK_HISTORY_PERFORMANCE_REPORT_MVP.md`를 source of truth로 본다. 1차 지표는 `LockHistoryScreen` users / active blocked users, LockHistory repeat users, 반복 `app_block_intercepted`/session이며, #211 공유 CTA 전환은 후속/보조 지표로 분리한다. PR #485로 read model/UI/string/test slice는 `develop`에 반영됐고, 2026-06-05 code-lane instrumentation으로 `lock_history_performance_summary_viewed` / `lock_history_top_apps_viewed` 코드 계약도 추가됐다. payload는 `period_type`, `report_state`, count/duration/top-app-count bucket만 쓰고 앱 이름/package/raw session/raw timestamp는 금지한다. 아직 `origin/main`/production tag/Play deploy, GA4 Admin 등록/metadata 확인, 14일 관측 경계는 남아 있으므로 instrumentation 포함 버전 배포 전 이벤트 0건을 UX 실패로 해석하지 않는다.
...
- 2026-06-06T08:33:25Z metrics snapshot의 30일 합산에서는 `screen_view` `38,338` 중 `(not set)+blank` gap이 `24,627`(`64.2%`)였고, 최신 관측 version `1.7.7` active share도 `145 / 767 = 18.9%`로 `주의`다. 이 값은 위 14일 smoke와 직접 합산하지 않고, #13을 release/tag/Play deploy + D+14 재측정 전까지 닫지 않는 guardrail로만 둔다.
