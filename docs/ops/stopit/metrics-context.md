...
- LockHistory 성과 리포트(#465)는 `docs/LOCK_HISTORY_PERFORMANCE_REPORT_MVP.md`를 source of truth로 본다. 1차 지표는 `LockHistoryScreen` users / active blocked users, LockHistory repeat users, 반복 `app_block_intercepted`/session이며, #211 공유 CTA 전환은 후속/보조 지표로 분리한다. 새 `lock_history_performance_summary_viewed` / `lock_history_top_apps_viewed` 이벤트를 추가하더라도 `period_type`, `report_state`, count/duration bucket만 쓰고 앱 이름/package/raw session/raw timestamp는 금지한다. 구현·GA4 Admin 등록·release/tag/Play deploy·14일 관측 전에는 새 이벤트 0건을 UX 실패로 해석하지 않는다.
