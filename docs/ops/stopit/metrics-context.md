...
- 2026-06-12 live readback(`2026-06-12T00:09:20Z`)에서도 전체 `newUsers`가 570명으로 직전 30일 대비 +94.5%였지만 `Direct` 신규가 334명(58.6%)으로 유지됐고 `Organic Search` 신규는 236명으로 #65 기준선 178명을 넘은 상태이며 `sessions`는 5,524회로 직전 5,829회 대비 -5.2%다. 따라서 현재 신규 유입 반등은 #242 외부 확인 전까지 ASO 회복이 아니라 attribution 판정 보류 신호로 본다.
...
- 2026-06-12T00:09:20Z metrics snapshot의 30일 합산에서는 `screen_view` `46,933` 중 `(not set)+blank` gap이 `28,255`(`60.2%`)였고, 최신 관측 version `1.7.7` active share도 `282 / 829 = 34.0%`로 `충분`이다. 다만 관련 PR package의 main/tag/Play 포함 여부와 D+14 같은 쿼리 창 재측정 전까지 #13을 닫지 않는 guardrail로만 둔다.
