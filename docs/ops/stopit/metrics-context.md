...
- 반복 차단 기반 자동 루틴 제안(#531)은 `docs/REPEAT_BLOCK_ROUTINE_SUGGESTION.md`를 source of truth로 본다. #455보다 더 후행의 개인화 후보이며, 반복 시간대·요일·앱 카테고리 bucket을 로컬에서 해석해 기존 루틴이 커버하지 않는 패턴 1개를 루틴 prefill로 제안한다. 지표는 `repeat_block_routine_suggestion_clicked / shown`, `applied / clicked`, 적용 cohort의 D7/D30 루틴 사용·반복 차단 유지율이고, `surface`, `suggestion_reason`, `time_bucket`, `day_type`, `category_bucket`, `repeat_count_bucket`, `routine_coverage_state`, `suggestion_variant`만 허용한다. 구현/release/GA4 Admin 등록/readback 전 이벤트 0건은 수요 없음으로 해석하지 않는다.
