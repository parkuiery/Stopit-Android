...
- 2026-06-14T00:09:03Z metrics snapshot의 30일 `screen_view` 합산에서는 `(not set)+blank` gap이 `29,883 / 50,641 = 59.0%`였고 최신 관측 production version `1.7.7` active share도 `317 / 841 = 37.7%`(`충분`)였다. 이 값은 위 14일 query를 대체하지 않지만, #13 closure가 여전히 release/tag/Play deploy + D+14 재측정 경계에 있음을 확인하는 guardrail로 둔다.
