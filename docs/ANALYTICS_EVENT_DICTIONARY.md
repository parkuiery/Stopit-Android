...
- `KeepAnalyticsScreen.CANONICAL_SCREEN_NAMES`는 현재 코드가 인정하는 canonical `screen_view` set이다. 새 화면/route 추가 시 이 set과 screen_view 계약 표를 함께 갱신하고, 누락되면 #13 screen 품질 회귀로 본다.
