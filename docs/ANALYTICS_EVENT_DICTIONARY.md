...
- 2026-06-06T02:10:31Z metrics snapshot의 30일 `screen_view` 합산에서는 `(not set)+blank` gap이 `24,642 / 38,332 = 64.3%`였고 최신 관측 production version `1.7.7` active share도 `145 / 767 = 18.9%`(`주의`)였다. 이 값은 위 14일 query를 대체하지 않지만, #13 closure가 여전히 release/tag/Play deploy + D+14 재측정 경계에 있음을 확인하는 guardrail로 둔다.
