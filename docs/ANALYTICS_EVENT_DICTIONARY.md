...
- 반복 차단 루틴 추천 구현 계약: `app/src/main/java/com/uiery/keep/domain/repeatblock/RepeatBlockRoutineSuggestionPolicy.kt`, `app/src/main/java/com/uiery/keep/data/repeatblock/RepeatBlockRoutineSuggestionStore.kt`, `app/src/main/java/com/uiery/keep/ui/component/RepeatBlockRoutineSuggestionCard.kt`, `app/src/main/java/com/uiery/keep/feature/routine/RoutineNavigation.kt`, `app/src/main/java/com/uiery/keep/feature/routine/RoutineBottomSheetViewModel.kt`, `app/src/main/java/com/uiery/keep/analytics/KeepAnalytics.kt`, `app/src/main/java/com/uiery/keep/analytics/FirebaseKeepAnalytics.kt` (2026-06-06 code/QA lane에서 policy + analytics event contract, RoutineRoute prefill 적용, dismiss local store 추가; 2026-06-08 #651 code-lane에서 analytics boundary는 `RepeatBlockRoutineSuggestionAnalyticsPayload` DTO로 분리해 feature-local suggestion object를 analytics API/Firebase adapter가 직접 import하지 않는다. #987 code-lane에서 shared model/policy를 `domain.repeatblock`, dismiss store를 `data.repeatblock`으로 올려 app-root Block runtime이 `feature.routine.*` suggestion 타입/정책/store를 직접 import하지 않도록 고정했다. #1022 이후 Block/Home/LockHistory CTA card layout·CTA·dismiss hierarchy는 app shared `RepeatBlockRoutineSuggestionCard`가 소유하고 각 화면은 surface copy/navigation/store action만 주입한다. #531 구현 완료 surface는 `home`/`lock_history`/`performance_report`/`post_block_success`이며, PR #994 이후 full-coverage suppression + partial-coverage uncovered-app prefill 계약까지 docs/static contract에 반영됐다. release/tag/Play deploy 및 GA4 등록/readback 전까지 live event 0건은 수요 없음으로 해석하지 않는다.)
- 목표 잠금 코드 계약: `app/src/main/java/com/uiery/keep/domain/goallock/GoalLockPolicy.kt`, `app/src/main/java/com/uiery/keep/database/entity/GoalLockEntity.kt`, `app/src/main/java/com/uiery/keep/database/dao/GoalLockDao.kt`, `app/src/main/java/com/uiery/keep/data/goallock/GoalLockRepository.kt`, `GoalLockCreationViewModel.kt`, `GoalLockDetailViewModel.kt`, `GoalLockAnalytics.kt`, Home goal-lock card/read-model wiring, `KeepAccessibilityService` goal-lock attribution (policy/persistence/creation UI/navigation/Home/Accessibility blocking/detail update/early-end/Home completion/compact-height QA footholds are repo-internal `develop` 상태. release/tag/Play deploy, GA4 Admin metadata, D+14/D+30 readback 전까지 live `goal_lock_*` 0건은 adoption 부재로 해석하지 않는다.)
- 부모 모드 코드 계약: `app/src/main/java/com/uiery/keep/feature/parentmode/ParentModePolicy.kt`, `app/src/main/java/com/uiery/keep/feature/parentmode/ParentModeSetupViewModel.kt`, `app/src/main/java/com/uiery/keep/data/parentmode/ParentModeSessionStore.kt`, `app/src/main/java/com/uiery/keep/analytics/FirebaseKeepAnalytics.kt`, `app/src/main/java/com/uiery/keep/service/KeepAccessibilityService.kt` (PR #519로 순수 정책 + `parent_mode_*` analytics API, PR #584로 session persistence + `block_source=parent_mode` Accessibility decision foothold 추가, PR #913(`9d169449`)으로 Parent Mode setup 진입 시 `ParentModeSetupScreen` canonical `screen_view` 기록 추가)
- 긴급해제 설정 변경 코드 계약: `app/src/main/java/com/uiery/keep/feature/emergencyunlocksettings/EmergencyUnlockSettingsViewModel.kt`, `app/src/main/java/com/uiery/keep/datastore/EmergencyUnlockSettingsStore.kt`, `app/src/main/java/com/uiery/keep/analytics/KeepAnalytics.kt`, `FirebaseKeepAnalytics.kt` (PR #698 / merge commit `8c303d75204bf9b2b6ab1e0ed4c9b6d8e2489260`로 `emergency_unlock_settings_changed` / `emergency_unlock_manual_reset_requested` Android wiring과 privacy-safe JVM payload 보장이 `develop`에 반영됐고, PR #789 / merge commit `8b9f2793fd6b1a5aba9ebd893b0b5a6e4aaa9fd8`로 동일 값 재선택·duration no-op toggle은 change 이벤트를 만들지 않도록 고정됐다. 이번 follow-up 이후 daily refill mode manual reset no-op도 analytics 분모를 부풀리지 않는다. GA4 Admin 등록, release/tag/Play deploy, D+14/D+30 readback 전까지 live event 0건은 adoption 부재로 해석하지 않는다.)
...
- `KeepAnalyticsScreen.CANONICAL_SCREEN_NAMES`는 PR #1005(`b1aa97d`) 이후 현재 코드가 인정하는 canonical screen set이다. 새 화면/route 추가 시 이 set과 screen_view 계약 표를 함께 갱신하고(운영 shorthand: screen_view 표), 누락되면 #13 screen 품질 회귀로 본다. D+14 screen quality 재측정 전에는 canonical set 보강만으로 live 개선 완료를 주장하지 않는다.
...
2026-08부터 arm 단계는 타이머 완주 경로뿐 아니라 **수동 종료(`user_toggle_off`) 경로에서도** 평가된다(`ReviewPromptArmer`). 이때 세션이 `MANUAL_SESSION_MIN_MILLIS`(10분) 미만이면 `BelowManualSessionDuration`으로 기록되며, 이 값은 **성공 세션 카운터를 올리지 않은 스킵**이라는 점에서 다른 eligibility 실패와 다르다. 계약은 `docs/REVIEW_PROMPT_LIFECYCLE.md`의 "수동 종료 arm 경로" 절이 source of truth다. `reason`은 이미 GA4 custom dimension으로 등록돼 있어 값 추가에 별도 Admin 등록이 필요하지 않다.
...
Issue: #810. 루틴 저장 완료 계측은 #455 CTA click, #531 반복 차단 추천 prefill, 그리고 수동 루틴 생성의 “실제 저장 완료” 분모를 맞추기 위한 후속 analytics 계약이다. 수동 루틴 생성과 CTA/추천 prefill 저장 완료를 같은 저장 완료 분모로 묶는다. PR #813(`2c4d4ae`)로 Android wiring은 `develop`에 반영됐고, PR #828(`b6d8cbd`)로 Home 보조 CTA 진입은 `entry_surface=home_secondary`, `creation_source=post_first_block_cta`로 저장 완료 attribution을 넘긴다. PR #846(`6dce8e3`) 이후 런타임 bucket 값도 문서/GA4 계약의 `1`, `2_3`, `4_6`, `7`, `all_day`, `custom`, `disabled_user_choice` 경계와 맞춰졌다. 따라서 더 이상 문서 계약 전용 상태가 아니다. 다만 GA4 Admin 등록·release/tag/Play deploy·14일/30일 readback 전까지 live 0건을 루틴 생성 수요 없음으로 해석하지 않는다.

| 이벤트명 | 주요 파라미터 | 설명 |
| --- | --- | --- |
| `routine_saved` | `entry_surface`, `creation_source`, `selected_app_count_bucket`, `repeat_days_bucket`, `time_window_bucket`, `schedule_state` | 루틴 insert가 성공한 뒤 한 번만 남기는 generic 저장 완료 이벤트. CTA/추천/수동 생성 전환 분모로 사용한다. |

현재 enum/bucket 계약:

- `entry_surface`: `routine`, `home_secondary`, `home`, `lock_history`, `post_block_success`, `performance_report`, `repeat_block_suggestion`
- `creation_source`: `manual`, `post_first_block_cta`, `repeat_block_prefill`, `unknown`
- `selected_app_count_bucket`: `1`, `2_3`, `4_6`, `7_plus`
- `repeat_days_bucket`: `1`, `2_3`, `4_6`, `7`
- `time_window_bucket`: `morning`, `afternoon`, `evening`, `night`, `overnight`, `all_day`, `custom`
- `schedule_state`: `enabled`, `disabled_exact_alarm_missing`, `disabled_user_choice`, `disabled_unknown`
- `repeat_block_routine_suggestion_applied`는 추천 적용 이벤트로 유지하고, `routine_saved`는 generic 저장 완료 이벤트로 함께 전송한다. 두 이벤트는 같은 저장 성공에서 함께 발생할 수 있지만 의미가 다르다.
- `lock_scheduled`는 예약 성공 이벤트이므로 exact alarm 권한 부족으로 disabled 저장된 루틴의 저장 완료 분모로 쓰지 않는다. 권한 부족 저장은 `routine_saved.schedule_state=disabled_exact_alarm_missing`로만 구분한다.
- 금지 payload/query 축: routine name, app package/name/list, `lockApplications`, raw start/end time, routine id, raw history, raw timestamp, raw rendered text.

### 반복 차단 기반 자동 루틴 제안

반복 차단 패턴 기반 자동 루틴 제안의 제품/QA 계약은 `docs/REPEAT_BLOCK_ROUTINE_SUGGESTION.md`를 source of truth로 본다. MVP는 기존 LockHistory/차단 기록에서 반복되는 시간대·요일·앱 카테고리 신호를 로컬에서 계산하고, 기존 활성 루틴과 겹치지 않는 후보 1개만 루틴 생성 prefill로 제안한다. 2026-06-06 code/QA lane에서 `RepeatBlockRoutineSuggestionPolicy`, `repeat_block_routine_suggestion_*` analytics method/constant, `RoutineRoute`/`RoutineBottomSheetViewModel` prefill 적용, `RepeatBlockRoutineSuggestionStore` dismiss persistence 계약이 추가됐다. PR #561(`42b271f7`)로 Home/LockHistory CTA UI wiring과 apply/dismiss flow도 `develop`에 반영됐고, PR #835(`8bb6592`)로 Home active Goal Lock 상태에서 추천과 shown analytics를 suppress하며, PR #843(`a37a1558`)로 Home active emergency unlock runtime state에서도 추천과 shown analytics를 suppress한다. PR #887(`7202aaea`)로 LockHistory 성과 리포트 직후 추천 CTA의 shown/clicked/dismissed 및 Routine prefill entry surface도 `performance_report` 구현 표면으로 분리됐고, PR #899(`e083495d`)로 `rapid_retry` 후보 우선순위 회귀, PR #923(`19f5be8`)로 `post_block_success` 후보 산출·shown/dismissed/store가 추가됐다. PR #931(`2846e6b`) 이후 post_block_success card/clicked/prefill launch까지 연결됐고, PR #944(`21bdf65e`) 이후 Goal Lock-origin 차단(`block_source=goal_lock`)에서는 post_block_success 추천과 shown analytics를 suppress한다. PR #994 이후 full-coverage suppression + partial-coverage uncovered-app prefill 계약까지 문서/정적 계약에 반영됐다. 현재 구현 완료 surface는 `home` / `lock_history` / `performance_report` / `post_block_success`이며, active protection context는 추천보다 우선한다. 해당 UI wiring commit이 release/tag/Play deploy에 포함되기 전 live event 0건은 수요 없음이나 UX 실패로 해석하지 않는다. onboarding / pre-first-lock 사용자는 제외하고, 비난형 copy와 앱 이름/package/raw history/raw timestamp payload 및 dismiss store 저장을 금지한다.
...
| `surface` | 루틴 생성 CTA/반복 차단 추천 노출 표면 (`home`, `lock_history`, `performance_report`, `post_block_success`). #531에서 `performance_report`와 `post_block_success`는 repo-internal 구현 표면이며, 둘 다 release/tag/Play deploy와 GA4 Admin metadata/readback 전까지 live 0건을 수요 없음으로 해석하지 않는다. |
...
| Required | `entry_surface` | `entry_surface` | `routine_saved`, `emergency_unlock_step_viewed`, `emergency_unlock_validation_blocked`, `emergency_unlock_cancelled` | event별 진입 표면 비교. `routine_saved`는 `routine/home_secondary/home/lock_history/post_block_success/performance_report/repeat_block_suggestion`, 긴급해제 step analytics는 Lock/Block 진입 표면 계약을 각각 따른다. raw route/path 금지 |
| Required | `creation_source` | `creation_source` | `routine_saved` | 루틴 저장 완료가 수동 생성, post-first-block CTA, repeat-block prefill 중 어디서 온 전환인지 분리 |
...
- 2026-06-26T02:07:51Z metrics snapshot의 30일 `screen_view` 합산에서는 `(not set)+blank` gap이 `41,100 / 78,600 = 52.3%`였고 최신 관측 production version `1.7.7` active share도 `559 / 938 = 59.6%`(`충분`)였다. 이 값은 위 14일 query를 대체하지 않지만, #13 closure가 여전히 release/tag/Play deploy + D+14 재측정 경계에 있음을 확인하는 guardrail로 둔다.
