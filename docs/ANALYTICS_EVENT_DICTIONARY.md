...
| `surface` | 루틴 생성 CTA/반복 차단 추천 노출 표면 (`home`, `lock_history`, `performance_report`, `post_block_success`). #531에서 `performance_report`는 구현 표면이고, `post_block_success`는 PR #923 이후 후보 산출 + shown/dismiss analytics는 구현됐지만 카드 UI·clicked/applied·Routine prefill navigation은 미구현이다. |
