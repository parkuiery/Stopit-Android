...
- Failure classification: distinguish `artifact expired/missing`, `durable fallback missing`, and `provenance mismatch`. Expired/missing evidence can recover through the durable fallback; durable fallback missing is an operator recovery boundary; mismatch is a release safety blocker.
