...
- Android CI `stopit-prod-debug-apk` is a PR/smoke inspection artifact, not a release artifact. Keep its `retention-days: 7` shorter than signed release AAB artifacts (`30` days in Release Build / non-production Play Deploy) so repeated PR runs do not exhaust GitHub Actions artifact storage. If `Upload prod debug APK` fails with `Artifact storage quota has been hit`, classify it as an external GitHub Actions storage/quota boundary after the preceding build/test steps have passed; delete or let old artifacts expire, wait for GitHub's 6–12 hour quota recalculation window, then rerun the current-head Android CI check.
...
- Release candidates targeting `main` also run Android Release QA before merge:
  ...
  - `adb shell appops set com.uiery.keep SCHEDULE_EXACT_ALARM allow` 후 아래 allow/cancel 경로를 실행
