...
| --- | --- | --- | --- |
| CI | `.github/workflows/android-ci.yml` | PR/push to `develop` or `main`, manual | One merged Gradle verification (KDS + dev unit tests, dev lint, prod debug APK artifact) plus `Runtime evidence`, which verifies the locally recorded runtime gate result. The emulator `Runtime smoke gate` is `workflow_dispatch`-only. No signed release. No Play upload. |
