...
- The focused emulator runtime smoke gate runs on a developer machine via `scripts/runtime-gate.sh`, not on pull request CI. It writes `.runtime-evidence.json` recording each suite result plus a `runtime_digest` over the runtime sources (`RUNTIME_DIGEST_INPUTS` in `scripts/android_runtime_suites.py`). The Android CI `Runtime evidence` job recomputes that digest and rejects evidence that predates the current sources, so app/runtime changes cannot merge on a stale local run. The emulator job itself remains available as `workflow_dispatch` escalation.
