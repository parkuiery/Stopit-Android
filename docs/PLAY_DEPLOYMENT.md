...
- Release candidates targeting `main` also run Android Release QA before merge:
  ...
  - 위 deny gate는 exact alarm 재예약 실패 시 receiver 경로가 enabled 루틴을 `disabled=false`로 조용히 남기지 않고, `enabled=false` 강등 + `HAS_SHOWN_ALARM_PERMISSION=false` reset + no pending intent 계약을 지키는지 검증한다.
