...
- 2026-06-04T09:17:03Z 30일 metrics snapshot에서는 `screen_view` `35,550` 중 `(not set)`이 `23,616`(`66.5%`)였다. 같은 snapshot에서 최신 관측 version `1.7.7` active share는 `75 / 715 = 10.5%`라 #359 기준 `주의`다. 따라서 14일 smoke와 30일 snapshot 모두 #13을 “개선 완료”로 닫는 근거가 아니라 release boundary 전 guardrail이다.
- 2026-06-04 버전 채택률 smoke에서는 최신 관측 버전 `1.7.7` activeUsers가 75명, 전체 activeUsers가 715명으로 `75 / 715 = 10.5%`였다. #359 기준 `주의`이지만 30% 미만이라 #13/#14/#16/#307 관련 최신 PR 성과를 전체 30일 합산 지표로 판정하지 않는다.
...
- 2026-06-01/2026-06-04 스냅샷처럼 `Direct` 신규 비중이 커지거나 `Paid Search` 활성/세션만 남는 경우, ASO 효과 판정 전에 #242 attribution gate를 적용한다. 2026-06-04T09:17:03Z live readback 기준 전체 `newUsers`는 464명으로 직전 대비 +28.9%였지만 `Direct` 신규가 285명(61.4%)으로 유지됐고 `Organic Search` 신규는 179명으로 #65 기준선 178명을 간신히 넘은 정도다. `sessions`도 4,857회로 직전 6,350회 대비 -23.5%다. 즉 Play Console Search/Explore와 GA4 `Organic Search`가 같은 방향인지, external/campaign/UTM 누락이 아닌지 확인한 뒤 #65의 14일/30일 결론을 쓴다.
