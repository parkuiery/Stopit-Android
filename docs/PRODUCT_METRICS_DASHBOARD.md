...
- 2026-06-04T07:13:43Z 30일 metrics snapshot에서는 `screen_view` `35,550` 중 `(not set)`이 `23,616`(`66.5%`)였다. 같은 snapshot에서 최신 관측 version `1.7.7` active share는 `75 / 715 = 10.5%`라 #359 기준 `주의`다. 따라서 14일 smoke와 30일 snapshot 모두 #13을 “개선 완료”로 닫는 근거가 아니라 release boundary 전 guardrail이다.
