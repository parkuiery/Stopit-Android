...
- 2026-06-25T21:14:08Z 30일 metrics snapshot에서는 `screen_view` `77,263` 중 `(not set)+blank` gap이 `40,398`(`52.3%`)였다. 같은 snapshot에서 최신 관측 version `1.7.7` active share는 `554 / 874 = 63.4%`라 #359 기준 `충분`이다. 다만 관련 PR package의 `main/tag/Play 포함 여부`와 D+14 같은 쿼리 창 재측정 경계가 남아 있으므로, 14일 smoke와 30일 snapshot 모두 #13을 “개선 완료”로 닫는 근거가 아니라 release boundary 전 guardrail이다.
...
- 2026-06-26(+30일) 버전 채택률 snapshot에서는 최신 관측 버전 `1.7.7` activeUsers가 554명, screen_view 관측 전체 users가 874명으로 `554 / 874 = 63.4%`였다. #359 기준 `충분`이지만, #13/#14/#16/#307 관련 최신 PR 성과는 still `main/tag/Play 포함 여부`와 post-release 14일/30일 창을 확인한 뒤 판단한다.
...
- 2026-06-01/2026-06-07 스냅샷처럼 `Direct` 신규 비중이 커지거나 `Paid Search` 활성/세션만 남는 경우, ASO 효과 판정 전에 #242 attribution gate를 적용한다. 2026-06-25T21:14:08Z(+30일) live readback 기준 전체 `newUsers`는 684명으로 직전 대비 +208.1%였고 `Organic Search` 신규는 374명으로 #65 기준선 178명을 크게 넘었지만 `Direct` 신규도 310명(45.3%)으로 남아 있다. `sessions`도 7,392회로 직전 4,428회 대비 +66.9%다. 즉 Play Console Search/Explore와 GA4 `Organic Search`가 같은 방향인지, external/campaign/UTM 누락이 아닌지 확인한 뒤 #65의 14일/30일 결론을 쓴다.
