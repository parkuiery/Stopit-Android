...
2026-08-21 리텐션 병목 진단과 그에 따른 실행 계획은 `docs/RETENTION_DIAGNOSIS_2026_08.md`를 source of truth로 본다. 이 문서의 NSM(`주간 활성 차단 사용자 수`)은 그대로 유지하되, **병목의 위치는 D0 활성화가 아니라 D1–D7 리텐션 루프로 이동했다.** 특히 `first_lock_configured`와 `first_core_action_completed`의 D7 잔존이 각각 `7.7%` / `9.6%`로 전체 신규 기준선 `7.8%`와 사실상 차이가 없으므로, **활성화 전환율을 리텐션 대리 지표로 승격하지 않는다.** 같은 문서 1.1이 Amplitude `interval: 30` 창 스냅 함정의 실제 사례를 기록하므로 Amplitude 절대 수치를 인용하기 전에 먼저 읽는다.
