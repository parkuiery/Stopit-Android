...
- 2026-06-14T00:09:03Z 30일 metrics snapshot에서는 `screen_view` `50,641` 중 `(not set)+blank` gap이 `29,883`(`59.0%`)였다. 같은 snapshot에서 최신 관측 version `1.7.7` active share는 `317 / 841 = 37.7%`라 #359 기준 `충분`이다. 다만 관련 PR package의 `main/tag/Play 포함 여부`와 D+14 같은 쿼리 창 재측정 경계가 남아 있으므로, 14일 smoke와 30일 snapshot 모두 #13을 “개선 완료”로 닫는 근거가 아니라 release boundary 전 guardrail이다.
...
- 이번 code-lane follow-through로 Parent Mode setup 화면도 `ParentModeSetupScreen` canonical `screen_view` coverage에 들어갔다. 이 보강은 #471 Parent Mode setup 전환 해석의 선행 조건이지만, release/tag/Play deploy와 D+14 screen quality 재측정 전까지는 live 부모 모드 setup 화면 0건이나 `(not set)` 잔여분을 수요 없음/개선 완료로 해석하지 않는다.
- 2026-06-14 버전 채택률 snapshot에서는 최신 관측 버전 `1.7.7` activeUsers가 317명, 전체 activeUsers가 841명으로 `317 / 841 = 37.7%`였다. #359 기준 `충분`이지만, #13/#14/#16/#307 관련 최신 PR 성과는 still `main/tag/Play 포함 여부`와 post-release 14일/30일 창을 확인한 뒤 판단한다.
...
- 2026-06-01/2026-06-07 스냅샷처럼 `Direct` 신규 비중이 커지거나 `Paid Search` 활성/세션만 남는 경우, ASO 효과 판정 전에 #242 attribution gate를 적용한다. 2026-06-14T00:09:03Z live readback 기준 전체 `newUsers`는 578명으로 직전 대비 +111.7%였지만 `Direct` 신규가 335명(58.0%)으로 과다 상태를 유지했고 `Organic Search` 신규는 243명으로 #65 기준선 178명을 넘은 상태다. `sessions`도 5,778회로 직전 5,577회 대비 +3.6%다. 즉 Play Console Search/Explore와 GA4 `Organic Search`가 같은 방향인지, external/campaign/UTM 누락이 아닌지 확인한 뒤 #65의 14일/30일 결론을 쓴다.
- 2026-06-11 Play Console Cloud Storage `store_performance` readback은 #65의 병목을 더 좁혔다. `2026-04 -> 2026-05` store listing visitors(방문자)는 `1,112 -> 521`(-53.1%), acquisitions는 `319 -> 154`(-51.7%)였지만 CVR은 `28.7% -> 29.6%`(+0.9p)라서 5월 악화는 listing 전환율 하락이 아니라 store listing visitor/노출 감소로 본다. Traffic source CSV는 `Other 97.0%`로 thresholding되어 Search/Explore/external 원인 분리는 Play Console UI/후속 attribution readback 경계다.
